---
title: "Fire behaviour and ecological insights to inform refined carbon emissions estimates for Canadian wildfires"
author: "Thompson, D.K., Whitman, E., et al."
format: pdf
editor: visual
...
Note: what is target format and home for this? Is it too niche to be anything other than a info report, or something in *Carbon Balance and Management*?
...
### Relevant biomass pools of CBM

Short section explaining the pool definitions most relevant to fire.

### Logical structure

To simpllify the process of the creation of the DMs as a distillation of the complexities of fire severity and combustion patterns, the following logical axioms are proposed and maintained throughout:
...
### Canopy mortality from plot and remotely sensed data

Ellen to insert methods here

... the resulting field-based estimates of crown mortality by ecozone are as follows:
...
The other key ecozone and severity-dependent variable provided by field data is the area-wise fraction of completely unburned forest floor, which varies widely by severity class:
...
where DC is the Fire Weather Index Drought Code and FFFL is the Forest Floor Fuel Load (with ecozone averages given in [@letang2012] or site-level data). LL is the Litter Load, and is typically on the order of 0.2 kg/m2 for most boreal forest upland and peatland sites ([@thompson2017]). This distinction is necessary due to the flaming phase consumption of the litter layer as opposed to the smouldering phase consumption of deeper horizons. While ultimately this scheme can be used on individual fires with estimated or measured fuel loading and specific Drought Code values, here we use ecozone-averaged fuel loads and decadal composites of Drought Code to provide representative values. Specifically, a median Drought Code of detected fire hotspots in Canada from 2003-2021 using the same data as the Canadian CFEEPS-FireWork wildfire air quality model of [@chen2019] is presented below, along with proportional consumption values of the forest floor by ecozone:
...
Forest.Floor.Load <- Forest.Floor.Load - Litter.Load

Forest.Floor.consump.kg.m2 <- 0.016872*Median.DC^0.71*Forest.Floor.Load^0.671 - Litter.Load
...
kbl(FFFC, caption = "Fire Weather, fuel loading, and forest floor consumption values per ecozone") %>%
  kable_minimal()  %>%
          kable_styling(latex_options = "scale_down") %>%
  kable_styling(latex_options = "hold_position")
```

### Key calculations in fire C emissions

Go over 4-5 key equations: CFB From mortality, etc. Spell out the equations.
...
global.vars <- list(0.89, 0.004, 0.07, 0.79, 0.013, 0.16)
...
#here we use values directly from CFFEPS-FireWork
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 5 & SourceSink$SeverityClass == "Low"] <- 1 - ( SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "Low"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 6 & SourceSink$SeverityClass == "Low"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 7 & SourceSink$SeverityClass == "Low"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 8 & SourceSink$SeverityClass == "Low"])

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 5 & SourceSink$SeverityClass == "Mod"] <- 1 - ( SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "Mod"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 6 & SourceSink$SeverityClass == "Mod"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 7 & SourceSink$SeverityClass == "Mod"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 8 & SourceSink$SeverityClass == "Mod"])

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 5 & SourceSink$SeverityClass == "High"] <- 1 - ( SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "High"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 6 & SourceSink$SeverityClass == "High"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 7 & SourceSink$SeverityClass == "High"] + SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 8 & SourceSink$SeverityClass == "High"])
...
A classic checksum table to ensure all the major pools sum of up 1 is presented below (Boreal Plains, high severity), but keep in mind that the sum of combusted pools is longer precisely 1.0 since particulate matter and non-methane hydrocarbons are accounted as leaving the biomass pool but have no corresponding atmospheric pool at the moment:
...
### text here must exactly match the name of rows in SinkSource, however:
  ...
  sink.col.list <- c("Softwood Merchantable", "Softwood Stem Snag","Medium DOM","Medium Soil C","Softwood Foliage","Above Ground Very Fast soil C", "CO2", "CH4", "CO")
  source.row.list <- c("Softwood Merchantable", "Softwood Stem Snag","Medium DOM","Medium Soil C","Softwood Foliage","Above Ground Very Fast soil C", "CO2", "CH4", "CO")
  ...
  for (j in seq(1:length(sink.col.list))) {
    for (k in seq(1:length(sink.col.list))) {
      ...
      ifelse(is.na(SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$Source == source.row.list[j] & SourceSink$Sink == sink.col.list[k] & SourceSink$SeverityClass == SeverityClass]),NA,DM.temp[j,k] <- SourceSink$Value[SourceSink$Ecozone == "BP" & SourceSink$Source == source.row.list[j] & SourceSink$Sink == sink.col.list[k] & SourceSink$SeverityClass == SeverityClass])
...
","Medium Soil C","Softwood Foliage","Above Ground Very Fast soil C", "CO2", "CH4", "CO"), align = "c",caption = paste0(SeverityClass, " severity Disturbance Matrix in ", ecozone)) %>%
...
column_spec(1, width = "3cm") %>%
 ...
 ##example of function, for BP and moderate severity:
...
Compare high severity-only C estimates from prior DMs to those with mixed severity.

## Discussion

### Comparison to prior wildfire emissions estimates (assumption of 100% CFB or else based on FWI+spread day alone).

## Conclusions

## Data availability

FAIR principle statement
