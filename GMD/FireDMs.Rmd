...
```{r non-GWP-total-C-plot, echo=FALSE, fig.cap="\\label{fig:non-GWP-total-C-plot}Direct emissions by severity class: orange = high, message=FALSE, warning=FALSE, message=FALSE, warning=FALSE, message=FALSE, warning=FALSE, yellow = moderate, green = low severity.  Labelled acronyms \n are Reconciliation Units (see Figure 1) as Ecozone then Province, so BC is both Boreal Cordillera \n and British Columba. Labrador is distinct from the island of Newfoundland for Reconciliation Units,\n so 'Lab' and 'NFLD' is used.  Some small Reconciliation Units without unique NIR biomass pool data are not shown in this figure but are computed still from adjacent RU from the same ecozone.  Some RU with very small burned area are also not shown in this figure.",out.width="75%"}
...
```{r fig-DMs-vs-FBP-CALCS, echo=FALSE, include=FALSE}
library(cffdrs)
...
```{r Emissions-2023-by-RU, echo=FALSE, fig.cap="\\label{fig:Emissions-2023-by-RU}Comparison of CO2e emissions per hectare between this method at the 2025 National Inventory Report for the 2023 wildfires.  Dots are labelled by RU and coloured by ecozone (see Figure 1), message=FALSE, warning=FALSE, message=FALSE, warning=FALSE, message=FALSE, warning=FALSE, with dot size proportional to 2023 area burned.  The solid line is the 1:1 reference line, and the dashed line represents 25% deviation from a 1:1 relationship.", out.width="75%"}
...
```{r example-FFFC-vs-BUI-plot-nls-calcs, warning=FALSE, include=FALSE}
...
```{r FFFC-plot-next3, echo=FALSE, fig.cap="Isoline contours of equal organic soil layer consumption in kg/m2 as a function of BUI and the organic soil layer fuel load in kg/m2.  Field observations of burn from 547 field measurements across Canada from the Yukon-Alaska border to Manitoba", message=FALSE, warning=FALSE, out.width="75%"}
...
```{r FFFC-plot-next4, echo=FALSE, fig.cap="Forest floor consumption as a function of BUI across varying levels of forest floor fuel load (FFFL)", message=FALSE, warning=FALSE, out.width="75%"}
