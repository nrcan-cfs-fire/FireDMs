...
```{r Fig1-RU-ecozones-NOT-RUN, eval=FALSE, fig.cap="\\label{fig::Fig1-RU-ecozones-NOT-RUN}Reconciliation Units used in the CBM-CFS3 framework.", message=FALSE, warning=FALSE, include=FALSE, out.width="50%"}
...
```{r Fig-CBI-ecozones-map-chunk, echo=FALSE, fig.cap="\\label{fig:Fig-CBI-ecozones-map} Locations of field-based burn severity plots.", message=FALSE, warning=FALSE, out.width="75%"}
...
In this framework we do not use the CBI aggregated metric itself to infer biomass consumption, but rather compilations of burn severity ground plot measurements that include all the individual biomass pool measurements. Field studies with data describing biomass consumption fractions for individual biomass pools were available primarily from the northwestern boreal forest of Canada [@whitman2018]. Smaller datasets with eastern Canadian and Alaska data were also available [@parks2019], as were data from the northwestern conterminous United States in ecozones that overlap with Canadian forests [@saberi2022]. Detailed mortality and surface consumption data were not available for the Atlantic Maritime and Mixedwood Plains ecozones, so an estimate was made by subsetting data from @saberi2022 to a moderate basal area range (25-50 m^2^/ha) mature eastern mixedwood and conifer-dominated stands. For the Boreal Cordillera ecozone, data were similarly subset to only sites with stand heights \<15 m. Field data for large diameter mature hardwood trees was only available for the Boreal Plains from @whitman2018 (primarily aspen, *Populus tremuloides*), and are used in all other ecozones without modification.

Individual burn severity plots were assigned a severity class based on ground observations, and an ecozone-level median biomass pool consumption fraction was computed for each severity class for each biomass pool (see Figure \ref{fig:Fig-CBI-ecozones-map}). For ecozones without ground plot measurements, an adjacent and ecologically similar ecozone with observations was used. The field survey data on biomass combustion and mortality rates as stratified by severity class, forms a improved data-driven and ecologically-informed approach to fire direct C emissions estimations here termed FireDMs (Fire Disturbance Matrix-severity).
