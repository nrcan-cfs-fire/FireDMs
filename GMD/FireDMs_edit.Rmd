---
title: "Incorporating observed fire severity in refined emissions estimates for boreal and temperate forest fires in the carbon budget model CBM-CFS3 v1.2"
...
correspongdingauthors: Dan K. Thompson (daniel.thompson@nrcan-rncan.gc.ca)
output:
...
copyrightstatement: Copyright His Majesty the King in Right of Canada as represented by the  Minister of Natural Resources Canada. This work is available under the Open Government Licence - Canada (http://open.canada.ca/en/open-government-licence-canada).
appendix:
...
# Introduction
#Hello
Wildfire is on par with insects as the largest stand-replacing disturbance process in Canada's forest, impacting \~1–3 Mha of Canada's 367 Mha forested area in a typical year [@hanes2019]. In Canada's updated 54-year burned area record, ten years have exceeded 4 Mha of burned area (or approximately 1% of Canada's forest area) [@skakun2022]. The 2023 fire season in Canada burned a remarkable 15 Mha owing to extreme drought, severe fire weather conditions, and a prolonged fire season length [@jainDriversImpactsRecordBreaking2024].

Managed northern forest areas, adjacent to communities, have shown the effects of local suppression leading to a stand age bias towards older stands; though the effect is largely limited to a c. 25 km radius around these widely dispersed communities [@parisien2020]. More southern boreal forest with extensive suppression activities has historically seen evidence of a reduction in observed vs potential area burned [@cummingEffectiveFireSuppression2005] though this effect has likely been erased given the increasing frequency of extreme burning conditions @wangCriticalFireWeather2023 and corresponding record area burned [@jainDriversImpactsRecordBreaking2024].

Burned area in Canada is dominated by a relatively small number of very large fires, with 3% of fires constituting 97% of the burned area [@stocks2002]. Lightning-caused fires account for approximately half of all ignitions and approximately 80% of burned area, but no distinction is made between human and lightning ignition for forest sector GHG reporting purposes. Annual burned area mapping for GHG reporting in Canada is conducted using a composite of satellite and aerial mapping at 30 metre resolution [@hall2020]; the relatively small number of large fires, and their slow vegetation regeneration [@white2017] allows for reliable mapping using multispectral imagery such as Landsat within one year of the fire [@whitman2018].

Canada reports on GHG emission and removals from the forest sector in Canada’s National GHG Inventory Report (NIR). Carbon stocks and stock changes are modelled using the Carbon Budget Model of the Canadian Forest Sector (CBM-CFS3) [@kurzCBMCFS3ModelCarbondynamics2009] parameterized and driven by spatial and non-spatial data describing forest inventory, annual yield and disturbance [@stinsonInventorybasedAnalysisCanadas2011]. Following international good practice guidance to focus on human-caused emissions in NIRs, Canada focuses emission reporting on managed forest lands, which make up approximately 225 Mha of Canada’s 367 Mha total forest area. In CBM-CFS3, forest inventory is represented by spatially referenced stand lists representing large homogeneous stands that fall within spatial units (e.g. forest management areas) and broad regions called Reconciliation Units (RU) that are the intersection of ecozones with provincial/territorial boundaries [@kurz2002]. Within CBM-CFS3 precisely mapped burned areas are summarized at the level of spatial units and applied randomly to stands within that unit.

Large wildfire burned area is not distributed equally across differing forest management regimes in Canada. Managed forest, where GHG emissions reporting is mandated, accounts for 23% of the area burned between 1972 and 2024; these publicly-owned managed forest areas under long-term license to private timber companies forms 40% of Canada's forest area [@stinson2019]. Privately-owned forest constitutes 6% of the forest area, but only 0.5% of area burned. The large majority (c. 75%) of the forest area burned in Canada occurs in a mix of formally protected areas, remote unmanaged forest, Indigenous reserve lands and other uses without large-scale harvesting. In these areas, detailed GHG reporting is required only in protected areas.

Years with larger burned areas will improve the representative nature of random stands being assigned fire disturbance within a year, as a wider variety of stand types will be selected. Furthermore, large, drought-driven fire years such as 2023 have been observed to burn at more equal rates across the diversity of local ecosite flammability [@parks2018; @whitman2024] and thus provides an ideal test case for assessing changes in emissions estimates for a spatially referenced system. Importantly, these years with very large area burned still show a similar natural range in variability of burn severity [@whitman2018].

CBM-CFS3 currently assesses fire impacts to carbon pools only as representing high-severity fire, which is the most common of the three severity classes (low, moderate, and high) used for assessing of burned forest in Canada [@hall2008] and the conterminous US [@eidenshink2007]. Biomass consumption estimates in CBM-CFS3 are based on the assumption of 100% crown consumption of conifer fuels, with additional biomass consumption following a spatially-referenced aggregated estimate at the RU-level of annualized drought conditions. Biomass consumption estimates for the remaining forest carbon pools are made via the Canadian Fire Behaviour Prediction System (FBP; @forestrycanadafiredangerratinggroup1992) based on the fire's start date fire weather conditions. Though the FBP is able to simulate fire behaviour along the entire intensity gradient from surface to crown fire, the lack of fire severity mapping and biomass consumption rates along at different severity classes has constrained carbon modelling to the most common high severity class.

Quantification of the change in carbon stock in CBM-CFS3 is made via a “Disturbance Matrix” (hereafter referred to as a DM) which is a matrix of coefficients between 0 and 1, where rows are donor pools, and columns: receptor pools or gases that are generated by a fire. Atmospheric fluxes of CO~2~, CO, and CH~4~ are also tracked in the DM. Importantly, unlike emissions-only fire models commonly used in air quality modelling in Canada [@chen2019], DM definitions in CBM-CFS3 also track the transfer of live biomass to dead, but still uncombusted pools, such as the transfer from live stemwood pools (which are killed but not burned) to standing deadwood, also known as snags.

Although DMs may provide a more conservative estimate of C emissions due to not combusting all biomass instantaneously, it may overestimate the C emissions due to assuming uniform fire severity. In Canada’s forests, a combination of climate, disturbance history, soils, and less frequently topographic variables determine leading tree species; at local scales (1--100 ha), tree species composition plays a major role in determining ecosystem susceptibility to fire [@bernier2016] where older, conifer-dominated forests burn at very high rates relative to adjacent deciduous or mixed stands. Large drought-driven fire years may increase burn rates in deciduous and mixed forest to levels similar to older conifer stands[@parksNewMetricQuantifying2014]. However, even in more intense drought conditions, deciduous and mixed forest burn rates are still consistently lower than in most conifer forests - except xeric conifer forest which tend to be more fire resistant [@whitman2018]. Thus, important local biases in fire activity and severity towards older and moderate to poorly-drained forests are not resolved in the spatially-referenced CBM-CFS3 when a uniform fire severity is applied.

To support recent advances in operational burn severity mapping for Canada [@whitman2020] alongside multi-decade reliable burned area records that provide certainty on fire growth start and end dates based on satellite hotspot detections [@guindon2014; @guindon2020; @hall2020], this paper describes a local scale (30-m) method for defining a per-pixel proportional carbon stock change and emissions estimate via a locally calculated fire DM. In this document, we outline the evidence-based fire DM updated and designed for a planned, spatially-explicit update to the CBM-CFS3, anchored in a three severity class paradigm (low, moderate, and high severity). These fire carbon flux models are built from a blend of aggregated field data linked to remotely sensed severity, as well as insights from fire physics and experimental fires. Key knowledge gaps are also highlighted, with interim solutions presented until further quantification can be done in field studies, such as from further wildfire observations, experimental fires, or prescribed fires.

Simplified fire DMs (i.e. a scalar reduction on 100% mortality assumption across all DM components, constant severity across all fires) have been used in valuable scenario exercises using a spatially-explicit version of CBM-CFS3 for assessment of future fire and harvest scenarios [@smyth2022]. Here, we provide a data-driven and regionally-adjusted framework that better reflects the ecological nuances of moderate and low-severity fire across Canada’s diverse ecozones. This paper describes the technical development of a fire severity module for the CBM-CFS3 model, version 1.2, benchmarked against the existing fire approach in the NIR and against independent C emissions observation during Canada's record fire season in 2023.
...
##load provinces just for the labels
PT <- st_read("~/nrcan-cfs-fire/FireDMs/Canada-provinces.gpkg", quiet = TRUE)
...
##make a new column called "name" for labelling matching the name column of PT_3978
ReportingUnits <- ReportingUnits %>% mutate(name=as.character(unit_id))

PT_RU <- bind_rows(PT_3978,ReportingUnits)


```

```{r Fig1-RU-ecozones-NOT-RUN, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, out.width="50%"}
...
ggplot(data = ReportingUnits) +  geom_sf(aes(fill=factor(eco_name)),show.legend = "polygon") + scale_fill_manual(values=ecozones_colours) +  geom_sf(data = lakesCanada_3978_clip,colour="black",fill="white")  +ylab("")+xlab("") + theme_bw() +theme(legend.title=element_blank(),legend.position = "inside",legend.position.inside = c(0.64,0.88),legend.text = element_text(size = 8),legend.key.size = unit(0.2, 'cm')) + guides(fill = guide_legend(ncol = 3),legend.background = element_blank()) + geom_label_repel(data=PT_RU,size=2.5,aes(label=name,geometry=geom),stat="sf_coordinates",max.overlaps = Inf) 

###  + geom_label_repel(data=ReportingUnits,nudge_y = c(0,0,0,0,250000,0,0,-100000,0,0,+100000),size=2.5,label.padding = unit(0.15, "lines"),aes(label = factor(unit_id),geometry=geom),stat="sf_coordinates")

###


ggsave("RU_plot.png",height=8,width=8,units=c("in"))

```

```{r Fig1-RU-ecozones, echo=FALSE, fig.cap="\\label{fig::Fig1-RU-ecozones} Colours corresponds to ecozones, and labels to sub-national provinces and territories. The smaller provinces of New Brunswick (NB), Nova Scotia (NS), and Prince Edward Island (PEI) are not fully spelled out in the labels. Reconciliation Units (RUs) used in the CBM-CFS3 framework are all unique combinations of ecozones and provinces/territories. Biomass estimates used in this analysis are reported as averages at the RU scale. Numbers labelled correspond to RU identifiers used in the text. ", message=FALSE, warning=FALSE, out.width="95%"}
...
kbl(SW.Mort.wide, caption = "Softwood fractional mortality by ecozone, as derived from median values from field studies in BP, TSW, MC, and PM ecozones.  See Section 2.1.5 for details.") %>%
  kable_minimal()#  %>%
          #kable_styling(latex_options = "scale_down") %>%
  #kable_styling(latex_options = "hold_position")
```

Numerous process-driven [@michaletz2006] or empirical [@hood2017] tree mortality models are available that show significant skill in predicting tree mortality based on fire behaviour (i.e. flame length, rate of spread). Since the driving data in this model is the mapped fire severity over the landscape scale, fire behaviour metrics such as flame length or scorching height of bark are not available as a continuous mapped product. Instead, softwood and hardwood overstory mortality is calculated per ecozone as a function of satellite-observed fire severity using the aggregated ground plot data (Table \ref{tab:tab-MortByEcozone}).

And since large-diameter, live trees killed by fire do not experience significant live stemwood consumption [@talucci2024], the entirety of the live stemwood biomass pool that is killed is transferred to the snag pool. Note that the field data and disturbance modelling undertaken here only accounts for tree mortality within the calendar year of the fire, and delayed mortality of over one year has been documented in boreal low and moderate severity fires [@angers2011] where less than half of total mortality occurs after the year of the fire. Thus, the modelling here does not account for delayed mortality that may extend upwards of 5 years after fire.
...
The consumption of live bark biomass is calculated in FireDMs, and consumption rates can be defined by severity class. At the moment, lacking robust field data on bark biomass consumption rates across ecozones and severity classes (which are a small portion of the overall biomass), the bark proportional consumption rate is set to 34% of the overstory mortality rate [@santín2015].
...
A major distinction is made between softwood and hardwood trees, where in Canada's boreal forests, a large fraction of hardwood trees (see Appendix B) are able to resprout even when the main stem has been killed by an intense forest fire [@brown1987]. Accordingly, the root mortality rates differ greatly between softwoods and hardwoods, with softwood root mortality equal precisely to stem mortality, while in resprouting hardwoods, little root mortality is observed even after intense fire [@pérez-izquierdo2019]. Though spatially explicit versions of CBM-CFS3 can resolve a species list down to the pixel level, currently an ecozone-level regional average composition of hardwood species with resprouting traits ("ReSproutFactor") is used and is shown in Table \ref{tab:tab-ReSproutByEcozone}.

The fraction of fine roots contained within the combustible forest floor layers (the variable "Prop.Fine.Root.duff" in the model ) can be a close to or exceeding 50% of the fine root biomass [@strong1985]; this fine root live biomass is assumed to burn alongside the organic soils, [@benscoter2011]. Fine root allocation is between the organic soil (termed "aboveground" or AG in CBM-CFS3 vs "belowground" or BG for mineral soils) is a parameter that can be ecozone level and is distinct for hardwoods (HW) vs softwoods (SW). As a result, the calculation for softwood fine root consumption and stemwood mortality rate (HW.Mort, as observed from field plots) are as follows, using hardwood as an example:
...
The litter layer forms the first biomass pool in which a spreading fire consumes fuel. In low-severity fires, the litter layer is consumed, but underlying duff material and live trees largely remain intact[@hessburg2019]. In these low-severity patches, some areas of fully unburned forest floor is present, and is summarized from CBI plot data by severity class and ecozone in Table \ref{tab:tab-UnBurnedForestFloorByEcozone}. Logically, since litter consumption is largely required for the ignition of the underlying duff layer, this unburned litter areas informs and constrains duff consumption.

### Snag and stump consumption

Dead standing trees and branches on average is a biomass pool smaller in size compared to organic soils in much of Canada's forest, but in areas of extensive insect-killed trees, recent fires, and blowdown, can be a substantial biomass pool. Compared to live stemwood of the same diameter, the low moisture content of snags and snag branches allows for much greater consumption during the passage of an intense flaming front [@stocks2004]. The snag branch pool experiences almost complete combustion in high severity fires [@talucci2019], and is highly correlated with CFB [@degroot2022]. The largest dead wood pool is the standing dead stemwood pool, which remains at most approximately 50% consumed; the consumption of standing snags pool is not quantified precisely in the severity plot data available. In the absence of extensive field data, snag consumption is estimated as:
...
##reading in the 2025 NIR data
NIR2023_AverageCPools <- read_csv("./NIR2025_AverageCPools.csv")
...
FFFC <- as.data.frame(cbind(Ecozones.fffc,Median.BUI,round(Duff.Load.kg.m2,digits=1),round(Duff.consump.kg.m2,digits = 1),round(Duff.consump.frac,digits = 2)))
...
kbl(FFFC,caption="Example fire weather, fuel loading, and duff consumption values per ecozone. Note that FFFL values are from the soil-specific Letang et al., (2012) compilation and not directly from the RU values provided in the National GHG Inventory Report.Median Buildup Index of burning is from Barber et al. (2024)", col.names = c("Ecozone", "Median Buildup Index of Burning", "Median FFFL kg m-2", "FFFC kg m-2", "Proportion consumed")) %>%  
  kable_minimal()  #%>%
          #kable_styling(latex_options = "scale_down") %>%
  #kable_styling(latex_options = "hold_position")
```

While ultimately this scheme can be used on individual fires with estimated or measured fuel loading and specific BUI values, as an example, an ecozone-averaged fuel load and a multi-year average of BUI during active fire satellite detection can also be used to provide representative values to showcase the model framework. Specifically, a median BUI of detected fire hotspots in Canada from 2003-2021 [@barber2024] using the same data as the Canadian CFEEPS-FireWork wildfire air quality model of [@chen2019] is presented in Table \ref{tab:tab-DuffConsumpTable}, along with proportional consumption values of the forest floor by ecozone.
...
## Source data for emissions comparisons

Direct wildfire C emissions from the proposed FireDMs scheme in CBM-CFS3 are compared to the FBP System model, which is observed biomass consumption rates from numerous experimental wildfires across Canada. The FBP System uses fixed fuel loads per fuel type, while FireDMs uses regional fuel loads, as represented by biomass pools defined per RU from forest inventory data.

Since FireDMs also distinguishes flaming vs smouldering combustion, the Modified Combustion Efficiency is used to compare modelled vs observed flaming to smouldering ratios:

\begin{equation}
MCE = \frac{CO_{2}}{CO_{2}+CO}
\end{equation}

where an MCE over 0.90 indicates a dominance of smouldering, and an MCE below 0.85 indicates a dominance of smouldering [@akagi2011]. While MCE is often defined in volumetric gas concentration units (ppmv), in this study we compute MCE as an output of FireDMs on a molar basis. Calculations of MCE from FireDMs is compared to localized smoke plume observations of MCE for distinct periods during the ARCTAS airborne campaign [@hornbrook2011].

While not strictly a carbon emission from wildfire, N~2~O is tracked as part of the NIR process and is an important component of CO~2~-equivalent calculations. CO~2~-equivalent calculations here will use the CBM-CFS3 emission factor of 0.17 g/kg CO~2~ (0.09 g/kg biomass) to simplify comparisons of C emissions between NIR and FireDMs, recognizing that this N~2~O emissions factor is lower than widely cited emissions factors compilations [@binteshahidNEIVAv10NextgenerationEmissions2024; @andreaeEmissionTraceGases2019] between 0.20 and 0.24 g/kg of biomass.

## Calculation of annual direct C emissions from fire

In an effort to compare model outputs against an independent, large-scale set of fire atmospheric emissions, fire direct atmospheric emissions from the record breaking Canadian 2023 fire season were computed using the above framework. The classified burn severity product from the National Burned Area Composite annual production was utilized (see @hall2020 for an algorithm description). In NBAC, an internal tracking value "NFIREID" is utilized, which is the final satellite-derived burned area polygon. Fire polygons are split across any subnational administrative boundaries (i.e. provinces and territories). Since carbon pool sizes vary across RU boundaries, this allows for a single NFIREID to be present across multiple RUs.

A total of 2199 fires as small as 0.09 ha (one 30 by 30 m Landsat pixel) were mapped in NBAC for burn severity for a total of 14.60 Mha. Only fires over 100 ha were utilized as a lower limit of where meaningful per-fire estimates of the fraction of low, moderate, and high severity burned area was available. Including only fires 100 ha and larger reduced the total number of fires to 966 but the total area remained largely the same at 14.58 Mha. A total of 189,704 ha of post-fire salvage logging was also mapped in 2023 and is assigned to the moderate severity class after consultation with provincial land managers. The direct C emissions from fire shown here are not altered by the act of post-fire salvage logging. Additionally, the NBAC mapping process accounts for unburned islands (and areas with a mapped fire severity no different than unburned) which count towards the total fire area but do not have a disturbance matrix and direct C estimate applied.

All thermal detection hotspots from VIIRS that intersect the NBAC perimeter were extracted from the historical hotspot archive that supports the Canadian smoke emissions model CFFEPS-FireWork [@chen2019], which provide Fire Weather Index values for each hotspot, including BUI. A unique DM was then calculated for each fire using the median BUI per fire across all observed hotspots.

To compute total direct fire emissions per fire, a single estimate of the carbon pool size based on the RU of the centroid of the NBAC polygon was applied. Fires that cross provincial boundaries are mapped as discrete fires on either side of the boundary in NBAC, but no such division of fires was made across ecozone boundaries. While spatially explicit biomass maps are available for some aboveground components [@guindon2024] and some organic soil components [@hanes2022], the majority of the required pool sizes in CBM-CFS3 for the computation of the fire DMs are available only at the spatially referenced RU scale.

To calculate the sum total of estimate direct C emission from Canadian wildfires in 2023, severity maps were created for each fire using 2024 season satellite observations using an approach that produces severity class maps using both classified spectral indices [@whitman2020] with additional fire environment covariates to predict severity class at 30-m. A single representative area-weighted Buildup Index value from fire hotspots was computed for each fire. A set of DMs (composed of one DM per severity class) was then applied to Reconciliation Unit carbon stock values for the total fire area falling into low, moderate, and high severity classes. The same Buildup Index was used for all severity classes per fire event when computing the DM per fire event. Per-fire direct C emissions (i.e. CO~2~, CO, CH~4~, PM~2.5~, etc.) were then computed and summed by ecozone and nationally.

RU-level carbon stocks were extracted from the National Forest Carbon Monitoring Accounting and Reporting System (NFCMARS) CBM-CFS3 model results for the start of 2023. With the severe drought and extreme fire weather conditions of the 2023 fire season [@jainDriversImpactsRecordBreaking2024], patterns of burned area bias towards older conifer stands found in more typical fire seasons were instead replaced by unbiased burned area patterns that reflect the overall landscape composition [@boulanger2024]. Thus, the 2023 Canadian fire season provides an ideal case to test regionally-averaged carbon stock estimates.
...
VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*0.34
  ...
  #Notes Modification:
  ## refined proportions in the softwood and hardwood other pools for the components (D Thompson, 2026-02-11 after review of pools)
  
  VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.56
  VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.1
  VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.21
  VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.05
  VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.08
  
  VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.48
  VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.18
  VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.21
  VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.05
  VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] <- 0.08
...
#Notes Modification: Changed to SW.Mort so if split by cohort ensure has the correct proportion, added bark component  
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 1 & SourceSink$SeverityClass == "Low"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]))+
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 1 & SourceSink$SeverityClass == "Mod"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]))+
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 1 & SourceSink$SeverityClass == "High"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]))+
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Changed to SW.Mort so if split by cohort ensure has the correct proportion, added bark portion

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 2 & SourceSink$SeverityClass == "Low"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])+
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])* (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(1-VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 2 & SourceSink$SeverityClass == "Mod"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])+
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])* (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(1-VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 2 & SourceSink$SeverityClass == "High"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])+
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])* (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(1-VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Added fluxes 152-

#FluxID: 152-157 for combustion of bark in merch pool

#Source: SW Merch
#Sink: CO2
#PLS: CO2 emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 152 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCO2
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 152 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCO2
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 152 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCO2
  
#FluxID: 153
#Source: SW Merch
#Sink: CH4
#PLS: CH4 emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 153 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 153 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 153 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCH4
  
#FluxID: 154
#Source: SW Merch
#Sink: CO
#PLS: CO emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 154 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCO
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 154 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCO
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 154 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCO
  
#FluxID: 155
#Source: SW Merch
#Sink: PM25
#PLS: PM25 emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 155 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM25
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 155 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM25
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 155 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM25
  
#FluxID: 156
#Source: SW Merch
#Sink: PM10
#PLS: PM10 emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 156 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM10
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 156 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM10
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 156 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM10
  
#FluxID: 157
#Source: SW Merch
#Sink: NMOG
#PLS: NMOG emission from bark consumption of merch pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 157 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingNMOG
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 157 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingNMOG
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 157 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingNMOG
  
#FluxID: 4
#Source: SW Foliage
#Sink: Sw Foliage
#Plain Language Summary of flux: Green fraction of canopy remaining intact after fire
#Pseudocode: = 1-mortality
#Notes: 
#Notes Modification: Added Flux ID 4 here so that is calculated independently instead of leftover part of pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "Low"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "Mod"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 4 & SourceSink$SeverityClass == "High"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 8 & SourceSink$SeverityClass == "High"] <- global.vars$FlamingCO *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]
  ...
  #Source: SW Foliage
  #sink: PM25
  #PLS: PM25 emission from Crown Fraction Burned
  #Pseudocode: CFB*FlamingPM25
  #Notes: 
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 101 & SourceSink$SeverityClass == "Low"] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 101 & SourceSink$SeverityClass == "Mod"] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 101 & SourceSink$SeverityClass == "High"] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]
  
  
  #FluxID: 118
  #Source: SW Foliage
  #sink: PM10
  #PLS: PM10 emission from Crown Fraction Burned
  #Pseudocode: CFB*FlamingPM10
  #Notes: 
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 118 & SourceSink$SeverityClass == "Low"] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 118 & SourceSink$SeverityClass == "Mod"] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 118 & SourceSink$SeverityClass == "High"] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]
  ...
  #Source: SW Foliage
  #sink: NMOG
  #PLS: PNMOG emission from Crown Fraction Burned
  #Pseudocode: CFB*FlamingNMOG
  #Notes: 
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 135 & SourceSink$SeverityClass == "Low"] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 135 & SourceSink$SeverityClass == "Mod"] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 135 & SourceSink$SeverityClass == "High"] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]
...
#Source: SW Other
#sink: Sw Other
#PLS: Portion of other pool that remains after fire
#Pseudocode: 
#Notes: 
#Notes Modification: Added Flux ID 9 here so that is calculated independently instead of leftover part of pool, also changed stump fraction to burn like merch pools; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 9 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])* (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) + 
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]+
VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 9 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) + 
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]+
VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 9 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) + 
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]+
VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Adjusted calculation so that branches that die but are not combusted are included, also changed stump fraction to burn like merch pools; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 10 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] - VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
    
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 10 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] - VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 10 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] - VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "SW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * VarDefs$Value[VarDefs$Variable.Name == "SW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "SW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) +
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes:  works so far and gives reasonable values of ~0.06, 0.07, 0.11
#Notes Modification: Missing branch consumption here has been added, removed stump consumption to be only mortality; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 11 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCO2 

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 11 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCO2 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 11 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCO2 
...
#Notes Modification: Missing branch consumption here has been added; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 12 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCH4 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 12 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCH4 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 12 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCH4 
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 13 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCO 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 13 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCO 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 13 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCO 
     
  #FluxID: 102
  #Source: SW Other
  #sink: PM25
  #PLS: Proportional Combustion sum of branches, stumps, small trees and bark (mix of flaming and smouldering)
  #Pseudocode:(Bw.frac.of.other*CFB*LgBranch.Comb.Frac*FlamingPM25)+
  # (Stumps.frac.of.other*DOM.Comb.Rate*SmoulderingPM25)+
  # (Litter.frac.burned*SmTree.frac.other*FlamingPM25)+
  # (Bark.frac.other*Bark.Comb.Frac*FlamingPM25)
  #Note: SW submerch is not consumed in fire (becomes snag or survives) so is not represented here
  #Notes Modification: Added here to have calculation independent; changed branch portion so that large and small branches are included in calculation
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 102 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingPM25 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 102 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingPM25 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 102 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingPM25 
  ...
  #sink: PM10
  #PLS: Proportional Combustion sum of branches, stumps, small trees and bark (mix of flaming and smouldering)
  #Pseudocode:(Bw.frac.of.other*CFB*LgBranch.Comb.Frac*FlamingPM10)+
  # (Stumps.frac.of.other*DOM.Comb.Rate*SmoulderingPM10)+
  # (Litter.frac.burned*SmTree.frac.other*FlamingPM10)+
  # (Bark.frac.other*Bark.Comb.Frac*FlamingPM10)
  #Note: SW submerch is not consumed in fire (becomes snag or survives) so is not represented here
  #Notes Modification: Added here to have calculation independent; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 119 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingPM10 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 119 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingPM10 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 119 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingPM10 
  ...
  #sink: NMOG
  #PLS: Proportional Combustion sum of branches, stumps, small trees and bark (mix of flaming and smouldering)
  #Pseudocode:(Bw.frac.of.other*CFB*LgBranch.Comb.Frac*FlamingNMOG)+
  # (Stumps.frac.of.other*DOM.Comb.Rate*SmoulderingNMOG)+
  # (Litter.frac.burned*SmTree.frac.other*FlamingNMOG)+
  # (Bark.frac.other*Bark.Comb.Frac*FlamingNMOG)
  #Note: SW submerch is not consumed in fire (becomes snag or survives) so is not represented here
  #Notes Modification: Added here to have calculation independent; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 136 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG +
VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingNMOG 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 136 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingNMOG 
    
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 136 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "SW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "SW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "SW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "SW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "SW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG + VarDefs$Value[VarDefs$Variable.Name == "SW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingNMOG 
...
#sink: Sw fine roots
#PLS: fine root survival
#Pseudocode:  =(1-SW mortality)(fine roots in duff * (1- duff consumption) + fine roots in mineral soil)
#Note:
#Notes Modification: Added Flux iD 22 here so that is calculated independently instead of leftover part of pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 22 & SourceSink$SeverityClass == "Low"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 22 & SourceSink$SeverityClass == "Mod"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 22 & SourceSink$SeverityClass == "High"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 27 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingCO
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 104 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 104 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 104 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 121 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 121 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 121 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 138 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 138 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 138 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
...
#Pseudocode: survival = 1-mortality
#Notes:  by axiom, survival = 1-mortality,
#Notes Modification: Added merch bark 

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 28 & SourceSink$SeverityClass == "Low"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]))+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 28 & SourceSink$SeverityClass == "Mod"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]))+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 28 & SourceSink$SeverityClass == "High"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]))+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone] * (1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]))*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: added merch bark
 ...
 SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 29 & SourceSink$SeverityClass == "Low"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])+
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 29 & SourceSink$SeverityClass == "Mod"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])+
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 29 & SourceSink$SeverityClass == "High"] <- 
(1-VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])+
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone]) * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Added fluxes 158-163 for combustion of merch bark
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 158 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCO2
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 158 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCO2
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 158 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 159 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 159 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 159 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 160 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingCO
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 160 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingCO
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 160 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingCO
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 161 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM25
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 161 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM25
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 161 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM25
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 162 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM10
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 162 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM10
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 162 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM10
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 163 & SourceSink$SeverityClass == "Low"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingNMOG
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 163 & SourceSink$SeverityClass == "Mod"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingNMOG
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 163 & SourceSink$SeverityClass == "High"] <- 
(VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.merch" & VarDefs$Ecozone == ecozone])*(VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingNMOG
  
#FluxID: 31
#Source: HW Foliage 
#sink: HW Foliage
#PLS: surviving foliage
#Pseudocode: 1 - mortality
#Notes: 
#Notes Modification: Added Flux ID 31 here so that is calculated independently instead of leftover part of pool
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 31 & SourceSink$SeverityClass == "Low"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 31 & SourceSink$SeverityClass == "Mod"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 31 & SourceSink$SeverityClass == "High"] <- 1 - (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Added here to have calculation independent
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 105 & SourceSink$SeverityClass == 'Low'] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Low']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 105 & SourceSink$SeverityClass == 'Mod'] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Mod']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 105 & SourceSink$SeverityClass == 'High'] <- global.vars$FlamingPM25 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'High']
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 122 & SourceSink$SeverityClass == 'Low'] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Low']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 122 & SourceSink$SeverityClass == 'Mod'] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Mod']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 122 & SourceSink$SeverityClass == 'High'] <- global.vars$FlamingPM10 *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'High']
  
  
  #FluxID: 139
  #Source: HW Foliage
  #sink: NMOG
  #PLS: NMOG emission from Crown Fraction Burned
  #Pseudocode: CFB*FlamingNMOG
  #Notes: 
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 139 & SourceSink$SeverityClass == 'Low'] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Low']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 139 & SourceSink$SeverityClass == 'Mod'] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Mod']
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 139 & SourceSink$SeverityClass == 'High'] <- global.vars$FlamingNMOG *  VarDefs$Value[VarDefs$Variable.Name == 'HW.CFB' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'High']
...
#Pseudocode: 
#Notes: 
##Notes Modification: Added Flux ID 36 here so that is calculated independently instead of leftover part of pool, changed stumps to be related to HW mortality; changed branch portion so that large and small branches are included in calculation


SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 36 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) +
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) + 
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]+
VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 36 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) +
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) + 
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]+
VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 36 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) +
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) + 
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]+
VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone] * (1- VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes Modification: Adjusted calculation so that branches that die but are not combusted are included; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 37 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] - VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 37 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] - VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 37 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] - VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])))*VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] +
VarDefs$Value[VarDefs$Variable.Name == "HW.SubMerch.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * VarDefs$Value[VarDefs$Variable.Name == "HW.submerch.frac.of.other" & VarDefs$Ecozone == ecozone]+
VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == ecozone] *(VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])+
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone] * (VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * (1 - VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])
...
#Notes:  works so far and gives reasonable values of ~0.06, 0.06, 0.08
#Notes Modification: Missing branch consumption here has been added; ; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 38 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 + VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCO2 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 38 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 + VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCO2 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 38 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingCO2+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO2 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCO2 
...
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCH4 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 39 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCH4 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 39 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingCH4+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCH4 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCH4 
...
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingCO 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 40 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingCO 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 40 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingCO+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingCO +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingCO 
  ...
  # (Litter.frac.burned*SmTree.frac.other*FlamingPM25)+
  # (Bark.frac.other*Bark.Comb.Frac*FlamingPM25)
  #Note: HW submerch is not consumed in fire (becomes snag or survives) so is not represented here
  #Notes Modification: Added here to have calculation independent; changed branch portion so that large and small branches are included in calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 106 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingPM25 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 106 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingPM25 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 106 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingPM25+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM25 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingPM25 
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 123 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingPM10 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 123 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingPM10 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 123 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingPM10+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingPM10 +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingPM10 
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 140 & SourceSink$SeverityClass == "Low"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]* global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) * global.vars$FlamingNMOG 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 140 & SourceSink$SeverityClass == "Mod"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]* global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG +
    VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) * global.vars$FlamingNMOG 
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 140 & SourceSink$SeverityClass == "High"] <-  
VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == ecozone]*VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]* global.vars$FlamingNMOG+
VarDefs$Value[VarDefs$Variable.Name == "HW.BW.frac.of.other" & VarDefs$Ecozone == ecozone] * VarDefs$Value[VarDefs$Variable.Name == "HW.CFB" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] * (VarDefs$Value[VarDefs$Variable.Name == "HW.LgBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(1-VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone]) + VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*(VarDefs$Value[VarDefs$Variable.Name == "HW.SmBranch.frac.of.tot.BW" & VarDefs$Ecozone == ecozone])) * global.vars$FlamingNMOG +
VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == ecozone]* (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) * global.vars$FlamingNMOG 
...
#sink: HW fine roots
#PLS: fine root survival
#Pseudocode:  =(1-HW mortality*(1-resprout))(fine roots in duff * (1- duff consumption) + fine roots in mineral soil)
#Note: 
#Notes Modification: Added Flux ID 49 here so that is calculated independently instead of leftover part of pool

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 49 & SourceSink$SeverityClass == "Low"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"] *(1-VarDefs$Value[VarDefs$Variable.Name == 'HW.ReSprout' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Low']))* (VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 49 & SourceSink$SeverityClass == "Mod"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"] *(1-VarDefs$Value[VarDefs$Variable.Name == 'HW.ReSprout' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'Mod']))* (VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 49 & SourceSink$SeverityClass == "High"] <- (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"] *(1-VarDefs$Value[VarDefs$Variable.Name == 'HW.ReSprout' & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == 'High']))* (VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]*(1 - as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) + (1 - VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == ecozone]))
...
#Notes Modification: Added here to have calculation independent
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 108 & SourceSink$SeverityClass == 'Low'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 108 & SourceSink$SeverityClass == 'Mod'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 108 & SourceSink$SeverityClass == 'High'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 125 & SourceSink$SeverityClass == 'Low'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 125 & SourceSink$SeverityClass == 'Mod'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 125 & SourceSink$SeverityClass == 'High'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 142 & SourceSink$SeverityClass == 'Low'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 142 & SourceSink$SeverityClass == 'Mod'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 142 & SourceSink$SeverityClass == 'High'] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == ecozone]*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])) * global.vars$SmoulderingNMOG
...
#Notes Modification: Added here to have calculation independent
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 109 & SourceSink$SeverityClass == "Low"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 109 & SourceSink$SeverityClass == "Mod"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 109 & SourceSink$SeverityClass == "High"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 126 & SourceSink$SeverityClass == "Low"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 126 & SourceSink$SeverityClass == "Mod"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 126 & SourceSink$SeverityClass == "High"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 143 & SourceSink$SeverityClass == "Low"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 143 & SourceSink$SeverityClass == "Mod"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 143 & SourceSink$SeverityClass == "High"] <- (1 -  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$FlamingNMOG
...
#Notes Modification: should be 1-CWD frac for the part that has duff consumption

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 60 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + (1-VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(1-as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 60 & SourceSink$SeverityClass == "Mod"] <-  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + (1-VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(1-as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 60 & SourceSink$SeverityClass == "High"] <-  VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + (1-VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(1-as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))
...
#Notes Modification: modifed here so that is calculated independently instead of leftover part of pool

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 61 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO2
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 61 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 61 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 62 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 62 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 62 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCH4
...
#PLS: Combusted Fine and small woody debris plus dead coarse roots in the forest floor, approximately ≥5 and <75 mm diameter
#Pseudocode: = flux60*smouldering
#Notes Modification: modified here so that is calculated independently instead of leftover part of pool

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 63 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 63 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 63 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingCO
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 110 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 110 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 110 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 127 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 127 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 127 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 144 & SourceSink$SeverityClass == "Low"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 144 & SourceSink$SeverityClass == "Mod"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 144 & SourceSink$SeverityClass == "High"] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == ecozone])*(as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone]))))*global.vars$SmoulderingNMOG
...
#Notes Modification: did not match pseudocode, added unburned litter area, also assumed unburned litter area also remains as medum DOM

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 65 & SourceSink$SeverityClass == "Low"] <- (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) + VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 65 & SourceSink$SeverityClass == "Mod"] <- (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) + VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 65 & SourceSink$SeverityClass == "High"] <- (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(1 - VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) + VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 66 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 66 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 66 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 67 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 67 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 67 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 68 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 68 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 68 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingCO
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 111 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 111 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 111 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 128 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 128 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 128 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 145 & SourceSink$SeverityClass == "Low"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 145 & SourceSink$SeverityClass == "Mod"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 145 & SourceSink$SeverityClass == "High"] <-  (1 - VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*(VarDefs$Value[VarDefs$Variable.Name == "MedDOM.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$SmoulderingNMOG
...
#Notes Modification: should be the area that is burned or 1- unburned litter area in the calculation

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 70 & SourceSink$SeverityClass == "Low"] <- 1-(((as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])))*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 70 & SourceSink$SeverityClass == "Mod"] <- 1-(((as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])))*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]))

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 70 & SourceSink$SeverityClass == "High"] <- 1-(((as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])))*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]))
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 71 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 71 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 71 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 72 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingCH4
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 72 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingCH4
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 72 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 73 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 73 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 73 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingCO
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 112 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 112 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 112 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 129 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 129 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 129 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 146 & SourceSink$SeverityClass == "Low"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"])*global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 146 & SourceSink$SeverityClass == "Mod"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"])*global.vars$SmoulderingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 146 & SourceSink$SeverityClass == "High"] <- as.numeric(FFFC$Duff.consump.frac[FFFC$Ecozone == ecozone])*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"])*global.vars$SmoulderingNMOG
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 77 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 77 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 77 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 78 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 78 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 78 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 79 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 79 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 79 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCO
  ...
  #sink: PM25
  #PLS: Uncombusted fraction of conifer snags (still upright)
  #Pseudocode: =SW.Snag.Comb.Frac*Flaming
  #notes: snags not remaining upright nor fallen down are combusted
#Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 113 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 113 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 113 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 130 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 130 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 130 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 147 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 147 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 147 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingNMOG
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 83 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 83 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 83 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 84 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 84 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 84 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 85 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 85 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 85 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCO
  ...
  #sink: PM25
  #PLS: Combusted fraction of snag branches
  #Pseudocode: = (SW.Snag.BW.Comb.Frac)*Flaming
  #notes: 
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 114 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 114 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 114 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 131 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 131 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 131 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 148 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 148 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 148 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingNMOG
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 88 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 88 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 88 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 89 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 89 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 89 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 90 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 90 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingCO

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 90 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingCO
  
 
  #FluxID: 115
  #Source: Hardwood Stem Snag
  #sink: PM25
  #PLS: Uncombusted fraction of hardwood snags (still upright)
  #Pseudocode: =HW.Snag.Comb.Frac*Flaming
  #notes: snags not remaining upright nor fallen down are combusted
  #Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 115 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 115 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 115 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 132 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 132 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 132 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 149 & SourceSink$SeverityClass == "Low"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]) *global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 149 & SourceSink$SeverityClass == "Mod"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]) *global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 149 & SourceSink$SeverityClass == "High"] <- (VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]) *global.vars$FlamingNMOG
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 94 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 94 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCO2

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 94 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCO2
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 95 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 95 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCH4

SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 95 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCH4
...
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 96 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingCO
 
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 96 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingCO
  
SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 96 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingCO
  ...
  #sink: PM25
  #PLS: Combusted fraction of snag branches
  #Pseudocode: = (HW.Snag.BW.Comb.Frac)*Flaming
  #notes: 
#Notes Modification: Added here to have calculation independent
  
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 116 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 116 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingPM25
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 116 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingPM25
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 133 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 133 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingPM10
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 133 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingPM10
  ...
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 150 & SourceSink$SeverityClass == "Low"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Low"]*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 150 & SourceSink$SeverityClass == "Mod"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "Mod"]*global.vars$FlamingNMOG
  SourceSink$Value[SourceSink$Ecozone == ecozone & SourceSink$FluxID == 150 & SourceSink$SeverityClass == "High"] <- VarDefs$Value[VarDefs$Variable.Name == "HW.Snag.BW.Comb.Frac" & VarDefs$Ecozone == ecozone & VarDefs$SeverityClass == "High"]*global.vars$FlamingNMOG
  
#Notes Modification: Removed the loops here for PM25, PM10, and NMOG 
...
#Notes: 
#Notes Modification: Moved to above for independent calculation
...
ExtractFireDM <- function(Ecozone_select="BSE",Severity_select="High",BUI_for_DM=70)
{
  #DC_for_DM=700
  #Ecozone_select="BP"
  ...
  ###now, need to redefine fluxes 11-13,23,25-27,38,40,50,53,54,60,70 here just for the given severity and ecozone but for the selected value of Duff.consump.frac
  #Notes Modification: Added fluxes that were now calculated independently above (9,36,22,49,61,62,63,71,72,73) and modified code to match above for fluxes (11,12,13,38,39,40,60); flux 41 was supposed to be flux 39, missing flux 52
 #Notes Modification: changed which fluxes are re-calculated based on changes above
  
  
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 22 & DM_temp$SeverityClass == Severity_select] <- (1- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select]) * (VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(1 - Duff.consump.frac.ExtractFireDM) + (1 - VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]))
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 23 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.Mort" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select] * VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(1 - Duff.consump.frac.ExtractFireDM)
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 25 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCO2
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 26 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCH4
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 27 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCO
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 104 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingPM25
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 121 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingPM10
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 138 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == "SW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingNMOG

   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 49 & DM_temp$SeverityClass == Severity_select] <- (1- VarDefs$Value[VarDefs$Variable.Name == "HW.Mort" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select] *(1-VarDefs$Value[VarDefs$Variable.Name == 'HW.ReSprout' & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select]))* (VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]*(1 - Duff.consump.frac.ExtractFireDM) + (1 - VarDefs$Value[VarDefs$Variable.Name == "HW.prop.Fine.Root.duff" & VarDefs$Ecozone == Ecozone_select]))
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 50 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.Mort' & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select] * (1-VarDefs$Value[VarDefs$Variable.Name == 'HW.ReSprout' & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(1 - Duff.consump.frac.ExtractFireDM)
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 52 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCO2
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 53 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCH4
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 54 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingCO
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 108 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingPM25
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 125 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingPM10
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 142 & DM_temp$SeverityClass == Severity_select] <- VarDefs$Value[VarDefs$Variable.Name == 'HW.prop.Fine.Root.duff' & VarDefs$Ecozone == Ecozone_select]*(Duff.consump.frac.ExtractFireDM) * global.vars$SmoulderingNMOG
   
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 60 & DM_temp$SeverityClass == Severity_select] <- (VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select]*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(1-Duff.consump.frac.ExtractFireDM)))
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 61 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingCO2
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 62 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingCH4
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 63 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingCO
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 110 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingPM25
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 127 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingPM10
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 144 & DM_temp$SeverityClass == Severity_select] <- ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select] + ((1 - VarDefs$Value[VarDefs$Variable.Name == "CWD.frac.of.AGFastDOM" & VarDefs$Ecozone == Ecozone_select])*(Duff.consump.frac.ExtractFireDM)))*global.vars$SmoulderingNMOG
  
  DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 70 & DM_temp$SeverityClass == Severity_select] <- 1-(((Duff.consump.frac.ExtractFireDM))*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select]))
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 71 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingCO2
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 72 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingCH4
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 73 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingCO
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 112 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingPM25
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 129 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingPM10
   DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 146 & DM_temp$SeverityClass == Severity_select] <- Duff.consump.frac.ExtractFireDM*(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*global.vars$SmoulderingNMOG
   
#####
#Notes Modification: removed below as are above now
#DM_temp$Value[DM_temp$Ecozone == Ecozone_select & DM_temp$FluxID == 39] <- 
#VarDefs$Value[VarDefs$Variable.Name == "HW.stumps.frac.of.other" & VarDefs$Ecozone == Ecozone_select] * Duff.consump.frac.ExtractFireDM * global.vars$SmoulderingCH4 + ((1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select])*VarDefs$Value[VarDefs$Variable.Name == "HW.SmTree.frac.of.other" & VarDefs$Ecozone == Ecozone_select]*global.vars$FlamingCH4) + (VarDefs$Value[VarDefs$Variable.Name == "HW.bark.frac.of.other" & VarDefs$Ecozone == Ecozone_select] * VarDefs$Value[VarDefs$Variable.Name == "Bark.Comb.Frac" & VarDefs$Ecozone == Ecozone_select & VarDefs$SeverityClass == Severity_select] * global.vars$FlamingCH4)
...
#Notes Modification: removed here as calculated independently above now 
...
#DM_temp$Value[DM_temp$FluxID == 22] <- 1-(sum(DM_temp$Value[DM_temp$FluxID >=23 & DM_temp$FluxID <=27]) + DM_temp$Value[DM_temp$FluxID ==104] + DM_temp$Value[DM_temp$FluxID == 121]  + DM_temp$Value[DM_temp$FluxID ==138])

#SW_Fine_root_sum <- 1-sum(DM_temp$Value[DM_temp$Source == "Softwood Fine Roots"],na.rm=TRUE)
#DM_temp$Value[DM_temp$FluxID == 22] <- SW_Fine_root_sum
...
#HW_Fine_root_sum <- 1-sum(DM_temp$Value[DM_temp$Source == "Hardwood Fine Roots"],na.rm=TRUE)

#DM_temp$Value[DM_temp$FluxID == 49] <- HW_Fine_root_sum

#DM_temp$Value[DM_temp$FluxID == 49] <- 1-(sum(DM_temp$Value[DM_temp$FluxID >=50 & DM_temp$FluxID <=54]) + DM_temp$Value[DM_temp$FluxID ==108] + DM_temp$Value[DM_temp$FluxID == 125]  + DM_temp$Value[DM_temp$FluxID ==142])
...
testDM1$Value[testDM1$FluxID==22] ### fraction of remaining SW roots in test DM1
testDM1$Value[testDM1$FluxID==49] ### fraction of remaining HW roots in test DM1
sum(testDM1$Value[testDM1$Source == "Softwood Fine Roots"],na.rm=TRUE)
sum(testDM1$Value[testDM1$Source == "Hardwood Fine Roots"],na.rm=TRUE)


testDM1 <- apply(testDM1,2,as.character)
write.csv(testDM1,"BSE_High_BUI50.csv")
...
### provide a random number for the ID if otherwise not provided
  ...
  ###now load the mean pool size from CBM 2025 (object name is still 2023 due to legacy):
    NIR2023_AverageCPools <- read_csv("./NIR2025_AverageCPools.csv") ### it is in the same dir as the rmd file now

EmissionsPerFire <- function(ID=1,Area_ha=1,EcoBoundaryName_select="Boreal Plains",AdminBoundaryName_select="Alberta",Low_frac=0.20,Mod_frac=0.35,High_frac=0.45,Median_BUI=70)
{
  ...
  NIR_AverageCPools_filter <- NIR2023_AverageCPools %>% filter(EcoBoundaryName==EcoBoundaryName_select,AdminBoundaryName==AdminBoundaryName_select,TimeStep==33) 
...
emissions_plot <- ggplot(emissions_grid_out_plot,aes(Median_BUI,SumC_direct_emissions,colour = Severity_uniform,linetype=Severity_uniform)) + geom_line(linewidth = 1.5) + facet_wrap(vars(EcoAdmin),labeller = labeller(EcoAdmin = label_wrap_gen(20))) + theme_bw() + ylab("Direct emissions from wildfire\n(t C/ha) not adjusted to GWP") + xlab("Buildup Index of burning") + theme(legend.position="none") + scale_colour_manual(values = c("#440154FF",  "#287C8EFF","#C7E202FF")) +
  scale_linetype_manual(values = c("solid","longdash","dotdash")) + theme(strip.text.x = element_text(size = 8)) +theme(plot.caption = element_text(hjust = 0.5)) +  scale_x_continuous(guide = guide_axis(n.dodge = 2)) + ylim(0,100) + theme(panel.grid.minor = element_blank()) + theme(strip.background = element_rect(colour = "black", fill = "white"))

ggsave("emissions_plot.png",width=8,height=8,units=c("in"))
```

```{r non-GWP-total-C-plot, echo=FALSE, fig.cap="\\label{fig::non-GWP-total-C-plot}Direct emissions by severity class: solid dark line = high severity, green dot-dash = moderate, blue dashed line = low severity.  Labelled acronyms \n are RUs named as ecozone then administrative unit, so BC is both Boreal Cordillera \n and British Columba. Labrador is distinct from the island of Newfoundland for Reconciliation Units,\n so 'Lab' and 'NFLD' is used.  Some small RUs without unique NIR biomass pool data are not shown in this figure but are computed still from adjacent RU from the same ecozone.  Some RU with very small burned area are also not shown in this figure.",out.width="100%"}

### was getting a strange error since the output map in pdf was too large for the latex compiler somehow.  Inserting the above ggplot image as png instead 
...
ggplot(emissions_grid_out,aes(Median_BUI,SumCO2e_direct_emissions_GWP,colour = Severity_uniform)) + geom_line() + facet_wrap(vars(EcoAdmin)) + ylab("Direct emissions from wildfire (Mg CO2-eq/ha)") + xlab("Buildup Index of burning")
```

Direct fire emissions per hectare by severity class and as a function of drought condition (BUI) are shown in Figure \ref{fig:non-GWP-total-C-plot}. Direct fire emissions per hectare were greatest (using a constant BUI of 200 as a practical upper limit) in the Pacific Maritime ecozone of British Columbia, with 83 and 71 t C/ha released in high and moderate severity fires, respectively. This equates to 295 and 252 t CO~2~-eq/ha and an MCE of 0.90 to 0.89, respectively. The Pacific Maritime ecozone features some of the highest relative contributions of canopy fuels to carbon release, with 32% of modelled emissions from the canopy, and 68% from the surface (i.e. woody debris, litter, duff, roots etc). Next highest emissions were 65 t C/ha from high severity fire in the Boreal Shield East (BSE) of Labrador which features far lower merchantable volume but overall softwood foliage pools only 5% smaller than the Pacific Maritime. Larger aboveground very fast DOM (i.e. litter) pools in the BSE of Labrador offset slightly smaller aboveground slow DOM (i.e. forest floor duff) pools as compared to the PM.

The lowest emissions per area were estimated to be in the Boreal Shield West of Saskatchewan, where total emissions of 16 and 22 t C/ha (56 and 76 t CO~2~-eq/ha) for low and moderate severity fire at a BUI of 200 were 77% in surface fuels. Despite much lower overall emissions, these low emissions fires still feature 77--79% of the total emissions from surface fuels. The Taiga Shield West of the Northwest Territories also was modelled to have emissions of only 21.5 t C/ha under low-severity fire.

Across both low and high severity fires, the effect of drought was fairly consistent, the emissions from BUI 200 fires (i.e. very dry conditions) were higher than those of identical severity class but at a much lower BUI of 40. This lower BUI of 40 represents the practical lower limit of large spreading fires in forests in Canada. Variations in BUI can lead to up to 60% enhancement in emissions as observed in Mixedwood Plains of Ontario and Quebec as well as the Atlantic Maritime of Nova Scotia and Prince Edward Island, but can as small as 10--13% enhancement in emissions per hectare in colder northwestern boreal regions such as the Taiga Shield West of the Northwest Territories, BSW of Alberta, and Taiga Cordillera of the Northwest Territories. This patterns is consistent across both high and low severity fire, with the BUI enhancement of emissions as high as 63% in low-severity fires, while the effect is only somewhat smaller in high severity fires at 54%. Regions with the highest sensitivity to drought, in terms of GHG emissions, tend to be those with higher softwood (i.e. broadleaf) forest composition near 50% (by volume) or more (i.e. Atlantic Maritime, Mixedwood Plains and also Boreal Plains of Manitoba). RUs with the highest aboveground slow DOM pools also showed similarly high drought sensitivity of \~42%. RUs with the lowest drought sensitivity tend to also be the RUs with the lowest overall emissions.

## Comparison against existing fuel consumption models

```{r fig-DMs-vs-FBP-CALCS, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
library(cffdrs)
...
NIR2023_AverageCPools_FBP_comp <- NIR2023_AverageCPools %>% mutate(EcoAdmin = paste0(NIR2023_AverageCPools$EcoBoundaryName," ",NIR2023_AverageCPools$AdminBoundaryName)) %>% filter(EcoAdmin == "Boreal Shield West Saskatchewan" | EcoAdmin == "Taiga Plains Northwest Territories" | EcoAdmin == "Taiga Shield West Northwest Territories" | EcoAdmin == "Boreal Plains Alberta"| EcoAdmin == "Montane Cordillera British Columbia" | EcoAdmin == "Boreal Shield East Quebec" ) %>% filter(TimeStep==33)
...
##TimeStep 33 in the NIR pools is end of 2022, so getting ready to analyze the 2023 fires on average

NIR2023_AverageCPools_temp <- NIR2023_AverageCPools %>% select(SPUID:TimeStep) %>% filter(TimeStep==33)
...
###here's a version with emissions intensity per hectare
AnnualEmissionsSummary <- AnnualEmissionsSummary %>% mutate(MgC_perha = SumC_direct_emissions/Area_ha)

#AnnualEmissionsSummary$NFIREID <- as.numeric(AnnualEmissionsSummary$NFIREID)

##optional write csv
write.csv(AnnualEmissionsSummary,"AnnualEmissionsSummary_2023_by_fire.csv",na="")
```

```{r Emissions-2023-by-RU-calc, echo=FALSE, message=FALSE, warning=FALSE}
...
colnames(AnnualEmissionsSummary_table) <- c("Jurisdiction","Ecozone","Reconciliation Unit","Area Burned (Mha)","Moderate Severity Fraction","High Severity Fraction","This Study Emissions t C/ha","This Study Emissions t CO2-eq/ha","NIR 2025 Emissions tCO2-eq/ha") 


ggsave("emissions2023.png")

```

```{r Emissions-2023-by-RU, echo=FALSE, fig.cap="\\label{fig::Emissions-2023-by-RU}Comparison of CO~2~-e emissions per hectare between the FireDMs estimate and current methodology, the 2025 National Inventory Report for the 2023 wildfires.  Dots are labelled by RU and coloured by ecozone (see Figure 1), with dot size proportional to 2023 area burned.  The solid line is the 1:1 reference line, and the dashed line represents 25% deviation from a 1:1 relationship.", out.width="75%"}


knitr::include_graphics("emissions2023.png")
```

```{r tab-RU-CO2e-comp, echo=FALSE}
kbl(AnnualEmissionsSummary_table,align="c", padding=0L,caption="Comparison of per hectare CO2e direct emissions from this study against the 2025 National Inventory Report for the 2023 wildfire season in Canada. Rows are sorted by decreasing 2023 area burned.  Note that for RUs with no managed forest area, the nearest forest inventory data available is used.") %>% kable_minimal() %>% column_spec(1:9, width = "2cm") %>%
  kable_styling(latex_options = "scale_down")  %>%
  kable_styling(latex_options = "hold_position")  
...
Comparison of emissions totals against the NIR is only possible for spatial units (i.e. Reconciliation Units, provinces, or ecozones) where the entire RU is composed of managed forest. In British Columbia, across 2.2 Mha burned, these FireDMs estimates were consistently lower than NIR. This was driven by a lower average emissions in FireDMs for both the Taiga Plains with 1.3 Mha burned (125 t CO~2~-eq/ha) and Montane Cordillera with 0.68 Mha burned (130 t CO~2~-eq/ha) as compared to the NIR (161 and 189 t CO~2~-eq/ha, respectively). For Alberta the 1.5 Mha of Boreal Plains ecozone burned showed close agreement (162 vs 163) as well as in the Taiga Plains (130 vs 144). The Taiga Plains fires of both Alberta and British Columbia were largely adjacent, and the FireDMs output was very similar (126 vs 130) across nearly identical moderate and high severity fractions, but the NIR showed a larger contrast (144 vs 161) that may originate from the underlying assumptions of the previous methods of calculating fire emissions in the NIR.

The RU with the largest burned area in 2023 was in the unmanaged forest of Quebec's Taiga Shield East, where NIR pool sizes from the adjacent jurisdiction of Labrador TSE were used in the FireDMs estimates, as no NIR pool sizes were available for Quebec's TSE ecozone RU, as there is no managed forest in that RU (the objective of the NIR being to report on GHG emissions in managed forest). Over this 2.5 Mha of burned area, the Quebec TSE FireDMs emissions estimates were substantially larger (147 t CO~2~-eq/ha) compared to NIR (99 t CO~2~-eq/ha, see Figure \ref{fig:Emissions-2023-by-RU}). In the Quebec Boreal Shield East ecozone where NIR pools are available, the FireDMs were still larger than NIR, though the gap between the two was much smaller (103 vs 92). Only the Manitoba Boreal Plains and Yukon Taiga Plains showed a similarly larger estimate in the FireDMs compared to NIR, with all other RUs showing a typically 10–30% reduction in emissions in the FireDMs compared to the NIR. Overall, this new emissions estimate method will yield more representative fire emissions estimates that are sensitive to annual differences in drought condition and fire-specific patterns of severity. The small overall impact is likely due to larger soil organic layer emissions in the FireDMs being offset by lower canopy consumption in the low and moderate severity areas.
...
This article was produced from a RMarkdown document with underlying data is available from the Zenodo repository http://dx.doi.org/10.5281/zenodo.17643211 [@thompson2025]. The disturbance matrices were produced in accordance with standards associated with the CBM-CFS3 version 1.2 (July 2025), which is available for download (after user registration) at https://ca.nfis.org/index_eng.html.  CBM-CFS3 is not open source, and is not re-archived here due to licensing restrictions. Only the free and open source @thompson2025 is required to produce the figures and calculations in this document, and all relevant CBM-CFS3 data for comparison are provided as tabular data in the @thompson2025 repository without the need to run or install CBM-CFS3. 
...
DKT, EW, and BH conceived the intitial framework. DKT wrote the algorithm. EW analyzed and compiled input data. HF, VMG, OH, CH, and BH assisted in model development and algorithm review. DKT, EW, and BH drafted the manuscript. All authors contributed manuscript revisions and approved of the final version.

**Acknowledgements**

The authors would like to thank Werner A. Kurz (retired) for initiating this project during his role as the head of Canada's forest carbon accounting team.
...
# Appendix: Disturbance Matrix Examples

Note that the pool source is shown in rows, and the destination pool is shown in the columns.

```{r example-DM-tables, echo=FALSE}
DMTables(ecozone = "BP", SeverityClass = "Low")
DMTables(ecozone = "BP", SeverityClass = "Mod")
DMTables(ecozone = "BP", SeverityClass = "High")
DMTables(ecozone = "TSW", SeverityClass = "High")

##loop this over severity classes and also BUI levels at increments of 10 from 10 through 200?

```

```{r BSE-DMs-DO-NOT-RUN, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

##example of producing DMs for a single ecozone across a number of BUI levels

BUI_levels <- seq(from=5, to=120,by=10)
Severity_levels <- c("Low", "Mod", "High")
  
DM_grid <- expand_grid(BUI_levels,Severity_levels)

for(i in seq(1:nrow(DM_grid)))
{
tmp_DM <- ExtractFireDM(Ecozone_select="BSE",Severity_select = DM_grid$Severity_levels[i],BUI_for_DM=DM_grid$BUI_levels[i])

tmp_DM$BUI <- DM_grid$BUI_levels[i]

if(i==1) DM_long <- tmp_DM else DM_long <- rbind(DM_long,tmp_DM)

}
DM_long <- as.data.frame(DM_long)

DM_long$DC <- DM_long$BUI*3.3 ##simple fixed-zero-intercept regression to estimate DC from BUI for fire growth days in the BSE from Barber et al's Canadian Fire Spread Database (R2 = 0.74)

DM_long <- apply(DM_long,2,as.character)

write.csv(DM_long,"BSE_DM_long.csv")

```

\newpage

# Appendix: List of Resprouting Hardwoods of Canada
...
# Appendix: Modelling of soil organic layer consumption
...
where FFFC is Forest Floor Fuel Consumption (SFC minus surface woody debris) in kg (biomass) m^-2^ and FFFL is Forest Floor Fuel Load in kg m^-2^. The forest floor as defined here is inclusive of the litter and duff layers, live mosses and lichens. This model presented above fits well within the dataset and extends the observed maximum FFFC to nearly 10 kg m^-2^. The ABoVE synthesis of FFFL and FFFC [@walker2020] expands upon a slightly smaller dataset used in a modelling summary also by @walkerFuelAvailabilityNot2020, where structural equation modelling was used to explore drivers of FFFC but no concise and readily reproducible modelling is produced. The results of the SEM from @walkerFuelAvailabilityNot2020 emphasized a greater role of FFFL over DC, though coarse reanalysis that lacked local fire agency weather stations was used. An analysis of just 2014 fires in the Northwest Territories by @walkerSoilOrganicLayer2018 showed that while the mean depth of burn across all black spruce stands was 6–10 cm, the driest (xeric) black spruce stands with the smallest FFFL showed upwards of 75% soil organic consumption, while deeper organic soils in subhygric black spruce stands showed less than 25% consumption.
...
The a parameter term that forms the exponent of *e* alongside the Buildup Index is related to the BUI value at which half of the maximum possible asymptotal consumption value is observed (for a given FFFL value). The NLS fitting was given a minimum value of -0.008 such that a 50% consumption rate was modelled as occurring at or around a BUI value of 70 for typical upland spruce forest floor (FFFL of 8 kg/m2 or AGSlow of 40 t C/ha) following the wildfire data of @degroot2009 as well as experimental fire observations in spruce forests from the FBP System. The other parameters were fit to the best possible value with no constraint.
...
```{r example-FFFC-vs-BUI-plot-nls-calcs, message=FALSE, warning=FALSE, include=FALSE}
...
FFFC_vs_BUI_plot <- ggplot(FFFC.single.final.df, aes(buildup_index,prop_sol_c_combusted)) + geom_line() + ylim(0,1) + ylab("Forest floor mass fraction consumed") + xlab("Buildup Index (BUI)") + theme_classic2()

ggsave("FFFC_vs_BUI_plot.png")
```

```{r example-FFFC-vs-BUI-plot-nls, echo=FALSE, fig.cap="Example of the forest floor consumption model for a duff layer (AGSlow) of 60 Mg C ha-1 (12 kg m-2)", message=FALSE, warning=FALSE, out.width="75%"}

### was getting a strange error since the output map in pdf was too large for the latex compiler somehow.  Inserting the above ggplot image as png instead 

knitr::include_graphics("FFFC_vs_BUI_plot.png")
```
...
```{r show-biplot-FFFC-new-calcs, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
FFFC_biplot <- ggplot(ABoVE_Data_w_FBP_excl_BC, aes(prop_sol_c_combusted,nls.pred.BUI)) + geom_point(aes(colour = factor(ecozone))) + scale_colour_manual(values=ecozones_colours_4) + geom_abline(intercept = 0, slope = 1) + geom_abline(intercept = 0,linetype = "dotdash", slope = 1.25) + geom_abline(intercept = 0,linetype = "dotdash", slope = 0.75)+ ylim(0,1) + xlim(0,1) + xlab("Observed fraction of forest floor mass loss") + ylab("Modelled fraction of forest floor mass loss") + theme_classic2()

ggsave("FFFC_biplot.png")
```

```{r show-biplot-FFFC-new, echo=FALSE, message=TRUE, warning=FALSE, fig.cap="Leave one out cross-validation of the forest floor consumption model.  Where multiple measurements were conducted within a single fire, the entire fire was excluded from the training data used in the model training. The dotted lines represent an error of plus and minus 25%", out.width="75%"}

### was getting a strange error since the output map in pdf was too large for the latex compiler somehow.  Inserting the above ggplot image as png instead 

knitr::include_graphics("FFFC_biplot.png")
```
...
```{r FFFC-surface-plot-calcs, message=FALSE, warning=FALSE, include=FALSE}
...
ggsave("FFFC_expand_plot.png")

```

```{r FFFC-plot-next, echo=FALSE, fig.cap="Isoline contours of equal organic soil layer consumption fraction as a function of BUI and the organic soil layer Carbon pool in Mg C/ha.  Field observations of burn from 547 field measurements across Canada from the Yukon-Alaska border to Manitoba.", message=FALSE, warning=FALSE, out.width="75%"}

### was getting a strange error since the output map in pdf was too large for the latex compiler somehow.  Inserting the above ggplot image as png instead 

knitr::include_graphics("FFFC_expand_plot.png")
```

\clearpage

```{r more-calcs, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
brks_FFFC_abs <- c(1,2.5,5,10,15,20)

FFFC_expand.BUI <- FFFC_expand.BUI %>% filter(FFFL < 25) %>% filter(buildup_index < 300)

plot_1 <- ggplot(FFFC_expand.BUI, aes(buildup_index, FFFL, z=FFFC)) + geom_contour(breaks=brks_FFFC_abs) + scale_y_continuous(limits = c(0,25)) + scale_x_continuous(limits = c(0,300)) + geom_dl(aes(label=..level..), method="maxvar.qp", stat="contour",breaks = brks_FFFC_abs)+xlab("Buildup Index") + ylab("Forest Floor Fuel Load (kg/m2)") + theme(plot.caption = element_text(hjust = 0)) 
...
plot_2 <- ggplot(FFFC_expand.BUI, aes(buildup_index, FFFC, z=FFFL)) + geom_contour(breaks=brks_FFFL_abs) + scale_y_continuous(limits = c(0,25)) + scale_x_continuous(limits = c(0,300)) + geom_dl(aes(label=..level..), method="maxvar.qp", stat="contour",breaks = brks_FFFL_abs) +xlab("Buildup Index") + ylab("Forest Floor Fuel Load (kg/m2") + theme(plot.caption = element_text(hjust = 0)) 

ggsave("plot_2.png")

FFFC_expand.BUI$FFFL <- as.factor(FFFC_expand.BUI$FFFL)

plot_3 <- ggplot(FFFC_expand.BUI, aes(buildup_index, FFFC)) + geom_point(aes(colour = FFFL)) +scale_color_viridis_d() + ylab("Forest Floor Fuel Consumption (kg/m2)") + xlab("Buildup Index")

ggsave("plot_3.png")

```

```{r FFFC-plot-next2, eval=FALSE, fig.cap="Isoline contours of equal organic soil layer consumption in kg/m2 as a function of BUI and the organic soil layer fuel load in kg/m2.  Field observations of burn from 547 field measurements across Canada from the Yukon-Alaska border to Manitoba.  Note that peatland ecosystems begin at approximately 50 kg/m2 of forest floor fuel load", message=FALSE, warning=FALSE, include=FALSE, out.width="75%"}
...
```{r FFFC-plot-next3, eval=FALSE, fig.cap="Isoline contours of equal organic soil layer consumption in kg/m2 as a function of BUI and the organic soil layer fuel load in kg/m2.  Field observations of burn from 547 field measurements across Canada from the Yukon-Alaska border to Manitoba", message=FALSE, warning=FALSE, include=FALSE, out.width="75%"}
...
```{r FFFC-plot-next4, eval=FALSE, fig.cap="Forest floor consumption as a function of BUI across varying levels of forest floor fuel load (FFFL)", message=FALSE, warning=FALSE, include=FALSE, out.width="75%"}
