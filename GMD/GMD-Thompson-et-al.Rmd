...
DM_temp$Value[DM_temp$FluxID == 70] <- (1 - (Duff.consump.frac.ExtractFireDM)) 
...
EmissionsPerFire <- function(ID,Area_ha=1,EcoBoundaryName_select="Boreal Plains",AdminBoundaryName_select="Alberta",Low_frac=0.20,Mod_frac=0.30,High_frac=0.30,Median_DC=250)
{
  ...
  Emissions_DF <- cbind(ID,Area_ha,EcoBoundaryName_select,AdminBoundaryName_select,Low_frac,Mod_frac,High_frac,Median_DC)
  ...
  Fluxes_summary <- Fluxes_all %>% group_by(SeverityClass,Sink) %>% summarize(FluxTotalMgCha = sum(FluxMgCha,na.rm=TRUE))
  ...
  Emissions_DF <- as.data.frame(cbind(Emissions_DF,MCE,CH4_MgC,CO_MgC,CO2_MgC,NMOG_MgC,PM10_MgC,PM25_MgC,ER_CO_CO2,ER_CH4_CO2))
  ...
  Emissions_DF <- as.data.frame(Emissions_DF) %>% mutate(across(5:15, as.numeric)) %>% mutate(across(5:15, round, 2))
...
```{r GasEmissions-all-combos, eval=FALSE, include=FALSE}
...
FireDM_grid <- expand(NIR2023_AverageCPools,nesting(EcoBoundaryName,AdminBoundaryName),DC_levels,Severity_classes)
...
emissions_temp <- EmissionsPerFire(ID=1,Area_ha=1,EcoBoundaryName_select = FireDM_grid$EcoBoundaryName[1],AdminBoundaryName_select = FireDM_grid$AdminBoundaryName[1],FireDM_grid$DC_levels[1])
