...
Similar to @degroot2009, Drought Code was a better predictor of FFFC (in this case the ratio of FFFC/FFFL) than Buildup Index as used in the FBP. In the logit transformed space, saturation-type non-linear curve using the relevant FWI component was fitted in a non-linear least squares model, but an additive term of the natural-logarithm transformed FFFC (given as Medium DOM in Mg C/ha) was used as well:
...
Along the parameter space of DC and Medium DOM (FFFL) in the dataset, the following fractional consumption values can be computed:

(show isolines of %consumed along DC 0-750 and MediumDOM 5-200 ish)
