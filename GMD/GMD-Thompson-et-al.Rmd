...
```{r non-GWP-total-C-plot, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(emissions_grid_out,aes(Median_DC,SumC_direct_emissions,colour = Severity_uniform)) + geom_line() + facet_wrap(vars(EcoAdmin)) + ylab("Direct emissions from wildfire (Mg C/ha) not adjusted to GWP") + xlab("Drought Code of burning") + geom_point(aes(x=Median_DC,y=Prop_surface*100))
```
...
```{r MCE-from-airborne, echo=FALSE, message=FALSE, warning=FALSE}
