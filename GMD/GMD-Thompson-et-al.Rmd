...
ABoVE_Data_w_FBP_excl_BC <- ABoVE_Data_w_FBP_excl_BC %>% filter(buildup_index>0)

ABoVE_Data_w_FBP_excl_BC$site[ABoVE_Data_w_FBP_excl_BC$burn_name=="HR16"] <- c("HR16")
ABoVE_Data_w_FBP_excl_BC$site[ABoVE_Data_w_FBP_excl_BC$burn_name=="HR17"] <- c("HR17")
...
```{r tab-CWDByEcozone, echo=FALSE, message=FALSE, warning=FALSE,tab.cap="\\label{tab:tab-DuffConsumpTable}"}
...
With the parameter constrained NLS fitting, the proportional consumption model for the forest floor has a leave-one-out (conducted at the fire-level, not plot) cross validated r2 of `r {round(cor(ABoVE_Data_w_FBP_excl_BC$prop_sol_c_combusted,ABoVE_Data_w_FBP_excl_BC$nls.pred.BUI)),digits=2)}`, and a Mean Percent Error of `r {round(MAE.nls/mean(ABoVE_Data_w_FBP_excl_BC$prop_sol_c_combusted,na.rm=TRUE)),digits=0)*100}`%
