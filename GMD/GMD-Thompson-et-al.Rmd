...
Table of MCE comparison from aircraft and tower to our calculated values
...
### Plot-level comparisons
  ...
  -across the nine ICFME fires, 62% of fuel consumption was at the surface (forest floor plus woody derbis).  The model schema estimates that 80% of the fuel consumption was at the surface, suggesting a bias towards higher surface fuel consumption. 
  ...
  which is 22.1 tC/ha or 4.42 kg/m2
...
(show perhaps what a few different a values look like?)
(also compute the DC value of about half the maximum FBP SFC values too, just for kicks)
...
# Appendix C: annual variability in observed Drought Code during wildfire spread, and impact on ecozone-level DM calculations

### (Currently not running, needs model update)

```{r AnnualDC, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r AnnualDC-Vis-DC50, eval=FALSE, fig.height=3, fig.width=4, message=FALSE, warning=FALSE, include=FALSE}
...
```{r AnnualDC-Vis-Consump, eval=FALSE, fig.height=3, fig.width=4, message=FALSE, warning=FALSE, include=FALSE}
...
# Appendix ###: DM template (can delete in final draft)
