...
ggplot(FFFC_Data, aes(Frac_load_consump,nls.pred)) + geom_point(aes(colour = factor(FuelType))) + geom_abline(intercept = 0, slope = 1) + ylim(0,1) + geom_abline(intercept = 0, slope = 1.25) + geom_abline(intercept = 0, slope = 0.75)

cor(FFFC_Data$Frac_load_consump,nls.pred)

###try a quick GAM as an alternative to the nls above, similar structure
FFFC.Frac.Consume.gam<-gam(Frac_load_consump_logit ~ s(BUI,k=4,bs="tp")+FuelType+FireType+s(Load,k=4,bs="tp"), data = FFFC_Data,method="REML",family=gaussian(link = "identity"), na.action=na.exclude)
summary(FFFC.Frac.Consume.gam)
plot.gam(FFFC.Frac.Consume.gam)

##not an amazing model, but lower consumption in M2 and higher in C4 makes some sense, as does higher in wildfire by a fair margin
##spline of BUI and fuel load makes sense, with fuel type and fire type in there too
### will try again when we add the AB data and any others too (Pelican, a few others), ABOVE data?
...
FFFC_Data <- rename(FFFC_Data, bg_c_prefire = Load)
...
FFFC_Data_trimmed <- FFFC_Data %>% filter(Site != "Dawson City") %>% filter(Site != "Green Lk") %>% filter(Site != "WBNP")

## assign ecoregion to those sites we are adding

ABoVE_Data_w_FBP <- bind_rows(ABoVE_Data,FFFC_Data_trimmed) 
...
ABoVE_Data_w_FBP <- ABoVE_Data_w_FBP %>% drop_na(prop_sol_c_combusted_logit) %>% filter(drought_code > 0) %>% filter(bg_c_prefire>0) %>% filter(ecoregion_name_l2!="Alaska Boreal Interior")#  %>%  filter(ag_c_prefire>0) %>% filter(prop_black_spruce>0) %>% filter(longitude>-140)
...
ABoVE.FFFC.Frac.Consume.gsl.nls <- gsl_nls(
  fn = prop_sol_c_combusted_logit ~ (drought_code^a)*(bg_c_prefire^b)+c,
  data = ABoVE_Data_w_FBP,
  start = list(a = 0.71, b = 0.671,c=0.1),
  algorithm = "lm",
...
### other candidates: 
 ...
 #fn = prop_sol_c_combusted_logit ~ (drought_code^a)*(bg_c_prefire^b)+c+(prop_black_spruce*d), ### only like a 4% improvement, but logical and sig increase in consumption with more black spruce, all else being equal
  #data = ABoVE_Data,
  #start = list(a = 0.71, b = 0.671,c=0,d=0.1),


summary(ABoVE.FFFC.Frac.Consume.gsl.nls)

gsl.nls.pred <- inv.logit(predict(ABoVE.FFFC.Frac.Consume.gsl.nls,newdata=ABoVE_Data_w_FBP))

ABoVE_Data_w_FBP <- cbind(ABoVE_Data_w_FBP,gsl.nls.pred)

ggplot(ABoVE_Data_w_FBP, aes(prop_sol_c_combusted,gsl.nls.pred)) + geom_point(aes(colour = factor(ecoregion_name_l2))) + geom_abline(intercept = 0, slope = 1) + ylim(0,1) + geom_abline(intercept = 0,linetype = "dotdash", slope = 1.25) + geom_abline(intercept = 0,linetype = "dotdash", slope = 0.75) + ylim(0,1) + xlim(0,1)

cor(ABoVE_Data_w_FBP$prop_sol_c_combusted,ABoVE_Data_w_FBP$gsl.nls.pred)

MAE.nls <- mean(abs(ABoVE_Data_w_FBP$prop_sol_c_combusted-ABoVE_Data_w_FBP$gsl.nls.pred))

MAE.nls/mean(ABoVE_Data_w_FBP$prop_sol_c_combusted)

##~44% error with the simple nls term for Canada-only data



# hist(ABoVE_Data$prop_sol_c_combusted_logit) #looks good and now normally distributed

FFFC.Frac.Consume.gam.ABoVE<-gam(prop_sol_c_combusted_logit ~ s(drought_code,k=4,bs="tp")+ecoregion_name_l2+s(bg_c_prefire,k=4,bs="tp"), data = ABoVE_Data,method="REML",family=gaussian(link = "identity"), na.action=na.exclude)
...
FFFC.tree <- rpart(prop_sol_c_combusted ~ drought_code + ecoregion_name_l2 + bg_c_prefire + prop_black_spruce + dob + ag_c_prefire,data=ABoVE_Data)
printcp(FFFC.tree)
rpart.plot(FFFC.tree)
FFFC.predict <- predict(FFFC.tree,newdata=ABoVE_Data)

cor(x=ABoVE_Data$prop_sol_c_combusted,y=FFFC.predict,method="pearson")

MAE.rpart <- mean(abs(ABoVE_Data$prop_sol_c_combusted-FFFC.predict))

### Mean percent error
MAE.rpart/mean(ABoVE_Data$prop_sol_c_combusted)
