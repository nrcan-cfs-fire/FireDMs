...
DM_temp$Value[DM_temp$FluxID == 70] <- (1-(Duff.consump.frac.ExtractFireDM) *(1-VarDefs$Value[VarDefs$Variable.Name == "Unburned.litter.area" & VarDefs$Ecozone == Ecozone_select]))
...
### provide a random number for the ID if otherwise not provided
  ...
  ###now load the mean pool size from CBM 2023:
    NIR2023_AverageCPools <- read_csv("../NIR2023_AverageCPools.csv") ### it is one directory above this Rmd file
...
## Comparison against observed fire gas emissions
...
-discussion: MCE modelled here fits well against obs MCE during peak burn periods, but does not represent smouldering-only periods such as night-time and after small rain events

### 2023 Canadian wildfire season total emissions

-Byrne et al 2024 using TROPOMI says 647 Mt C (570-727) but note their CO2:CO ratio is 7.7-10.0 and ours is more like 5.8 so we have more smouldering (<https://www.nature.com/articles/s41586-024-07878-z>). So 5.8/7.7 is 0.76, and 0.76\*570 (their lower range) is 435 Mt, so right in our range of 15 Mha x about 30 t C/ha

-ECMWF-Copernicus says 490 Mt C (<https://atmosphere.copernicus.eu/smoke-canadian-wildfires-reaches-europe>)

burn_severity_by_nfireid_ha.csv


```{r TotalEmissions2023, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
Severity2023$Mod_frac <- Severity2023$Moderate/(Severity2023$Unchanged.to.very.low+Severity2023$Low+Severity2023$Moderate+Severity2023$High+Severity2023$Post.fire.havest+Severity2023$NoData)
...
NBAC2023 <- NBAC2023 %>% filter(ADJ_HA>500) #this could be smaller, down to as little as 10 ha potentially
...
###weighted mean of DC:
weighted.mean(AnnualEmissionsSummary$Median_DC, AnnualEmissionsSummary$Area_ha,na.rm=TRUE)
...
-   ICFME was 4.2 kg/m2 total, and Median_DC of 363
...
which gives 23.0 t C/ha, this model predicts 4.6 kg/m2 (just using the mean pool values for NWT Taiga Plains)

-across the nine ICFME fires, 62% of fuel consumption measured on-site was at the surface (forest floor plus woody derbis). The model schema estimates that 80% of the fuel consumption was at the surface, suggesting a bias towards higher surface fuel consumption.

-   FBP comparison (broken down by admin/ecozones), especially C-3 burns since they don't have deep duff, C-1 also valid but super thin

like a table but here's it broken down:
...
SFC for Kenshoe Passive Crown fires was 0.88,1.54, and 1.07 respectively (1.16 avg), CFC was 0.95, 0.99,0.60 (using 1.71 which is the BW+foliage number from de Groot et al since it was likely some BW and some bark and all foliage) so TFC was 1.07+1.71
...
surface fires to match closest CFB == mortality class
