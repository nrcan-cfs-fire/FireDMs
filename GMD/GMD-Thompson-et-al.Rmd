...
!!!Ellen to insert methods here - including the figure of where the samples are from Figure \ref{fig:CBI-plots}

```{r Fig-CBI-ecozones-map, echo=FALSE, fig.cap="\\label{fig:CBI-plots} Locations of field-based Composite Burn Index measurements.", message=FALSE, warning=FALSE}
...
```{r tab-globalVarsEmissions, echo=FALSE, message=FALSE, warning=FALSE, tab.cap="\\label{tab:Emissions-factors}"}
...
ggplot(emissions_grid_out_FBP,aes(Median_BUI,SumC_direct_emissions,colour = Severity_uniform)) + geom_line() + facet_wrap(vars(EcoAdmin),labeller = labeller(EcoAdmin = label_wrap_gen(20))) + ylab("Direct emissions from wildfire (Mg C/ha) not adjusted to GWP") + xlab("Buildup Index of burning")  + scale_colour_manual(values = c("#fc6f03",  "darkgreen","gold")) + theme(strip.text.x = element_text(size = 8)) +theme(plot.caption = element_text(hjust = 0.5)) + ggnewscale::new_scale_color() +  geom_point(data=test_fbp_out,aes(Median_BUI,TFC*5,colour=CFB,shape=FuelType)) + theme(legend.position="bottom") +scale_colour_viridis_c()
...
kbl(NIR_AverageCPools_filterCanopy_sorted,caption="Carbon pool size (Mg C/ha) for canopy fuels by Reporting Unit for 2023. RU are sorted in decreasing size of Softwood Merchantable, the largest average surface pool.  Columns are in decreasing mean pool size.") %>%
    kable_styling(full_width = F, html_font = "Cambria") %>%
  kable_styling(latex_options = "scale_down")  %>%
  kable_styling(latex_options = "hold_position") 
...
ggplot(FFFC.single.final.df, aes(buildup_index,prop_sol_c_combusted)) + geom_line() + ylim(0,1)# + geom_point(data=subset(ABoVE_Data_w_FBP_excl_BC,AGSlow.Mg.C.ha>50 & AGSlow.Mg.C.ha<70)) + ylim(0,1)
...
```{r FFFC-plot-next2, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r FFFC-plot-next3, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r FFFC-plot-next4, echo=FALSE, message=FALSE, warning=FALSE}
 ###maybe like FBP with FFFC vs BUI, then varying lines of FFFL:
