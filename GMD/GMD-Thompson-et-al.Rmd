...
ABoVE.FFFC.Frac.Consume.gsl.nls <- gsl_nls(
  ...
  #fn = prop_sol_combusted_logit ~ (a*drought_code)+(exp(bg_c_prefire*b))+c, ### try an exponent on the load but not DC
  data = ABoVE_Data_w_FBP,
  start = list(a = -0.21, b = -0.0071,c=0.01),
  algorithm = "lm",
...
ABoVE.FFFC.Frac.Consume.gsl.nls.excl.BC <- gsl_nls(
  fn = prop_sol_combusted_logit ~ (1-exp(a*drought_code))*(bg_c_prefire*b)+c,
  data = ABoVE_Data_w_FBP_excl_BC,
  start = list(a = -0.071, b = -0.00671,c=0.1),
  algorithm = "lm",
  control = list(scale = "levenberg")
)


ABoVE.FFFC.Frac.Consume.gsl.nls.BC.only <- gsl_nls(
  fn = prop_sol_combusted_logit ~ (1-exp(a*drought_code))+(bg_c_prefire*b)+c,
...
summary(ABoVE.FFFC.Frac.Consume.gsl.nls.excl.BC)
...
ggplot(ABoVE_Data_w_FBP, aes(prop_sol_combusted,gsl.nls.pred)) + geom_point(aes(colour = factor(ecozone))) + geom_abline(intercept = 0, slope = 1) + ylim(0,1) + geom_abline(intercept = 0,linetype = "dotdash", slope = 1.25) + geom_abline(intercept = 0,linetype = "dotdash", slope = 0.75) + ylim(0,1) + xlim(0,1)

cor(ABoVE_Data_w_FBP$prop_sol_combusted,ABoVE_Data_w_FBP$gsl.nls.pred)

MAE.nls <- mean(abs(ABoVE_Data_w_FBP$prop_sol_combusted-ABoVE_Data_w_FBP$gsl.nls.pred))

MAE.nls/mean(ABoVE_Data_w_FBP$prop_sol_combusted)
...
FFFC.Frac.Consume.gam.ABoVE<-gam(prop_sol_combusted_logit ~ s(drought_code,k=4,bs="tp")+ecozone+s(bg_c_prefire,k=4,bs="tp"), data = ABoVE_Data_w_FBP,method="REML",family=gaussian(link = "identity"), na.action=na.exclude)
...
FFFC.tree <- rpart(prop_sol_combusted ~ drought_code + ecoregion_name_l2 + bg_c_prefire + prop_black_spruce + dob + ag_c_prefire,data=ABoVE_Data)
printcp(FFFC.tree)
