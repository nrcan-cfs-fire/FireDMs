...
Certain variables, like the paritioning of CO~2~:CH~4~:CO gas emissons, are constant throughout ecozones, but vary by flaming vs smouldering combustion modes. They are defined in a global variables table:
...
ggplot(emissions_grid_out,aes(Median_DC,SumC_direct_emissions,colour = Severity_uniform)) + geom_line() + facet_wrap(vars(EcoAdmin)) + ylab("Direct emissions from wildfire (Mg C/ha) not adjusted to GWP") + xlab("Drought Code of burning") + theme(legend.position="none") + scale_colour_manual(values = c("red", "blue", "gold")) + labs(caption=("Direct emissions by severity class: red = high, yellow = moderate, blue = low severity"))# + geom_point(aes(x=Median_DC,y=Prop_surface*100))
```
...
[[[compare against excess ash measurements from Turetsky (March 1999 fire in BSW SK) Patuanak, Sask. (55°51 N, 107°41 W) - note: should use DM values for high severity (unless we have it in NBAC) and nearest station DC (probably Buffalo Narrows). Check 2.2 kg C/m2 (22 t C/ha). This is a peatland, mind you]]]

Experimental fires at the International Crown Fire Modelling Experiment (Stocks et al., YYYY) provide a good example of detailed pre- and post-fire biomass measurements in pools that largely mirror the pools impacted by direct fire emissions modelled here.  The series of burns in the Taiga Plains ecozone of the Northwest Territories were conducted under a median Drought Code of 363.  The average Total Fuel Consumption (TFC) across the high severity crown fire burns was 4.2 kg/m2 of biomass consumption, accounting for foliage, branchwood, woody derbis, and forest floor consumption.  The corresponding estimate using the fire Disturbance Matrices constructed here predicts 4.7 kg/m2 of TFC, or 23.7 t C/ha. Across the nine ICFME fires, 62% of fuel consumption measured on-site was at the surface (forest floor plus woody derbis). The model schema estimates that 80% of the fuel consumption was at the surface, suggesting a bias towards larger litter and duff pool size or higher surface fuel consumption in the model compared to this single site.
