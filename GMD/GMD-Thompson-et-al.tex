...
\subsection{Comparison against observed fire gas emissions}
...
\hline
Hornbrook et al 2011 & 2008-07-01 & Afternoon (0.2Low 0.4Mod 0.4High) & Boreal Shield West & 250 & 0.92 & 0.909\\
\hline
...
-discussion: MCE modelled here fits well against obs MCE during peak
burn periods, but does not represent smouldering-only periods such as
night-time and after small rain events

\subsubsection{2023 Canadian wildfire season total emissions}

-Byrne et al 2024 using TROPOMI says 647 Mt C (570-727) but note their
CO2:CO ratio is 7.7-10.0 and ours is more like 5.8 so we have more
smouldering (\url{https://www.nature.com/articles/s41586-024-07878-z}).
So 5.8/7.7 is 0.76, and 0.76*570 (their lower range) is 435 Mt, so right
in our range of 15 Mha x about 30 t C/ha

-ECMWF-Copernicus says 490 Mt C
(\url{https://atmosphere.copernicus.eu/smoke-canadian-wildfires-reaches-europe})

burn\_severity\_by\_nfireid\_ha.csv

\subsubsection{Plot-level comparisons}

\begin{itemize}
\item
  compare against excess ash measurements from Turetsky (March 1999 fire
  in BSW SK) Patuanak, Sask. (55°51 N, 107°41 W) - note: should use DM
  values for high severity (unless we have it in NBAC) and nearest
  station DC (probably Buffalo Narrows). Check 2.2 kg C/m2 (22 t C/ha).
  This is a peatland, mind you.
\item
  ICFME was 4.2 kg/m2 total, and Median\_DC of 363
\end{itemize}
...
which gives 23.0 t C/ha, this model predicts 4.6 kg/m2 (just using the
mean pool values for NWT Taiga Plains)

-across the nine ICFME fires, 62\% of fuel consumption measured on-site
was at the surface (forest floor plus woody derbis). The model schema
estimates that 80\% of the fuel consumption was at the surface,
suggesting a bias towards higher surface fuel consumption.

\begin{itemize}
\tightlist
\item
  FBP comparison (broken down by admin/ecozones), especially C-3 burns
  since they don't have deep duff, C-1 also valid but super thin
\end{itemize}
...
all foliage) so TFC was 1.07+1.71
