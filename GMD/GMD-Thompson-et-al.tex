...
\item
  ...
  are not considered part of CFB.
\item
  The heat-killed but unconsumed fraction of the canopy = (mortality -
  CFB)
\item
...
Certain variables, like the paritioning of
CO\textsubscript{2}:CH\textsubscript{4}:CO gas emissons, are constant
...
{[}{[}{[}compare against excess ash measurements from Turetsky (March
1999 fire in BSW SK) Patuanak, Sask. (55°51 N, 107°41 W) - note: should
use DM values for high severity (unless we have it in NBAC) and nearest
station DC (probably Buffalo Narrows). Check 2.2 kg C/m2 (22 t C/ha).
This is a peatland, mind you{]}{]}{]}

Experimental fires at the International Crown Fire Modelling Experiment
(Stocks et al., YYYY) provide a good example of detailed pre- and
...
consumption, accounting for foliage, branchwood, woody derbis, and
...
on-site was at the surface (forest floor plus woody derbis). The model
...
fuel consumtion was only 40\% of TFC. Future iterations of this fire DM
...
EmissionsPerFire(ID=``Kenshoe Lake passive crown fires 5 9 12'',Area\_ha
= 1,EcoBoundaryName=``Boreal Shield
...
SFC for Kenshoe Passive Crown fires was 0.88,1.54, and 1.07 respectively
(1.16 avg), CFC was 0.95, 0.99,0.60 (using 1.71 which is the BW+foliage
number from de Groot et al since it was likely some BW and some bark and
all foliage) so TFC was 1.16+1.71
...
EmissionsPerFire(ID=``Sharpsands 11-17'',Area\_ha =
1,EcoBoundaryName=``Boreal Shield
