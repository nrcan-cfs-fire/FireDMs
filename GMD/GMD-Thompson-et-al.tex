...
\item
  current use of RU and average (or random draw) from table/list when
  used to determine C impacts of disturbance doesn't reflect the fact
  that fire's burn preferrentially in the boreal in older conifer stands
  \citep{bernier2016}.
\item
  Similarly, we also know that average severity is lower but also more
  variables in forest ecosystems like xeric boreal pines, as compared to
  higher and less variable severity in poorly drained spruce forests
\item
...
\hline
0.926 & 0.004 & 0.07 & 0.827 & 0.013 & 0.16\\
\hline
\end{tabular}
\end{table}

where CO\textsubscript{2} is responsible for 92.6\% of emissions in the
flaming phase, but only 82.7\% of emissions in the smouldering phase,
...
biomass consumption in the smouldering phase is 1.26 times higher in
...
\begin{verbatim}
### table with rows are CBM pools, columns are obs and
### modelled exp fires
...
exp.fire.table.defs <- c("SW.CFB", "HW.CFB", "SW.Mort", "HW.Mort",
    ...
    "HW.Snag.Comb.Frac", "MedDOM.Comb.Frac")
